...
async def seed_all_russian_cities(session: AsyncSession) -> int:
    ...
    try:
        # Загружаем все существующие города
        existing_res = await session.execute(select(City))
        existing_cities = existing_res.scalars().all()

        # Словари для быстрого поиска: по нижнему регистру имени и по слагу
        ...
        for city_data in RUSSIA_CITIES:
            ...
            # Если город уже есть в базе
            if name_key in by_name:
                city_obj = by_name[name_key]
                # Если у него нет слага — проставляем!
...
# ========== ОБНОВЛЕНИЕ СЛАГОВ (устаревшая, теперь не нужна, но оставлена для совместимости) ==========
...
async def upsert_station_fuel_price(
    ...
    Поддерживает русские и латинские названия топлива.
    """
    now = datetime.now(timezone.utc)

    # Универсальный маппинг русских и латинских названий топлива к FuelType Enum
    MAPPING = {
    ...
    if not fuel_type_enum:
        # Дополнительная попытка сопоставления по Enum
        for ft in FuelType:
    ...
    # Запрос с сортировкой по свежести, чтобы избежать MultipleResultsFound
    stmt = (
        select(FuelPrice)
        .where(
            and_(
