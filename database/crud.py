...
async def get_user(db: AsyncSession, telegram_id: int) -> Optional[User]:
    """Получает пользователя по telegram_id с подгрузкой города."""
    try:
    ...
    result = await db.execute(
        ...
        .options(joinedload(User.city))  # Явно подгружаем город
...
async def apply_referral(db: AsyncSession, new_user_id: int, referrer_code: str) -> bool:
    """Сохраняет реферала, но бонус начисляется только после первого поиска."""
    ...
    db.add(referral)
    await db.commit()
    # Сохраняем referrer_id в поле referred_by у нового пользователя
...
async def get_stations_in_radius(db: AsyncSession, city_id: int, lat: float, lon: float, radius_km: float = 10.0) -> List[Station]:
    """
    Возвращает станции в городе в квадрате radius_km вокруг координат (lat, lon)
    с использованием Bounding Box для ускорения.
    """
    ...
    ).options(selectinload(Station.prices))  # подгружаем цены для избежания N+1
    result = await db.execute(query)
    return result.scalars().all()
# ============================================================
...
async def aggregate_old_prices(db: AsyncSession, days_threshold: int = 60):
    """Удаляет цены старше days_threshold дней (для экономии места в БД)."""
