...
async def seed_all_russian_cities(session: AsyncSession) -> int:
    """Импорт 65+ городов РФ в БД без дубликатов."""
    added_count = 0
...
async def upsert_station_fuel_price(
    ...
    # Приводим fuel_type к Enum FuelType
    fuel_type_enum = None
    for ft in FuelType:
    ...
    if record:
        record.price = price
        record.source = SourceType.PARSER  # или другой источник
...
async def update_city_prices_by_brand(
    ...
    Если указан brand_pattern (например 'Татнефть' или 'Лукойл'), обновляет только их.
    Если не указан — обновляет все активные станции города.
    """
