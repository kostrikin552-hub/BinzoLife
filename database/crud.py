# database/crud.py — ПОЛНАЯ ВЕРСИЯ (все изменения из этапов 1–5)
import random
import string
import math
import logging
...
async def commit_or_rollback(session: AsyncSession) -> bool:
    """Гарантированный откат сессии при сбое транзакции."""
    try:
...
async def save_price(
    ...
    # Помечаем старые записи как is_fresh=False
    await db.execute(
        update(FuelPrice)
        .where(
...
async def activate_pro(db: AsyncSession, user: User, days: int = 30) -> User:
    ...
    # Стекинг: продлеваем от текущего срока, если он ещё активен
...
async def get_stations_in_radius(db: AsyncSession, city_id: int, lat: float, lon: float, radius_km: float = 10.0) -> List[Station]:
    ...
    ).options(selectinload(Station.prices))  # защита от MissingGreenlet
    result = await db.execute(query)
    return result.scalars().all()
...
async def set_first_search(db: AsyncSession, user_id: int):
    ...
    # Если есть реферал и ещё не награждён
    if user.referred_by:
        ...
        if referral:
            ...
            if referrer:
                ...
                await commit_or_rollback(db)
                # Уведомление рефереру (можно делать снаружи)
    await commit_or_rollback(db)
