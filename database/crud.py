# database/crud.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (все исправления)
import random
import string
import math
import logging
...
from utils.geocoder import reverse_geocode, geocode_address

logger = logging.getLogger(__name__)

# Проверка наличия библиотек таймзоны
try:
    ...
    logger.warning("Библиотеки timezonefinder или pytz не установлены. Таймзона пользователей не будет определяться автоматически.")
...
# ========== ИМПОРТ ГОРОДОВ (ИСПРАВЛЕННАЯ ВЕРСИЯ) ==========
async def seed_all_russian_cities(session: AsyncSession) -> int:
    """
    Безопасный сидинг 65+ городов РФ:
    1. Обновляет slug у уже существующих городов (если он был пуст)
    2. Добавляет недостающие города (без timezone_offset, проверяя и по slug, и по name)
    """
    added_count = 0
    updated_slugs_count = 0
    try:
        existing_res = await session.execute(select(City))
        existing_cities = existing_res.scalars().all()
        by_name = {c.name.strip().lower(): c for c in existing_cities if c.name}
        existing_slugs = {c.slug.strip().lower() for c in existing_cities if c.slug}

        for city_data in RUSSIA_CITIES:
            c_name = city_data["name"].strip()
            c_slug = city_data["slug"].strip().lower()
            name_key = c_name.lower()

            if name_key in by_name:
                city_obj = by_name[name_key]
                if not city_obj.slug:
                    city_obj.slug = c_slug
                    updated_slugs_count += 1
                    existing_slugs.add(c_slug)
            elif c_slug not in existing_slugs:
                new_city = City(
                    name=c_name,
                    slug=c_slug,
                ...
                added_count += 1
                by_name[name_key] = new_city
                existing_slugs.add(c_slug)

        await session.commit()
        logger.info(f"✅ Сидинг завершён: создано новых городов: {added_count}, обновлено слагов: {updated_slugs_count}")
    except Exception as e:
        await session.rollback()
        logger.error(f"❌ Ошибка при сидинге городов: {e}", exc_info=True)
        raise e
    return added_count

async def update_city_slugs_from_seed(session: AsyncSession) -> int:
    """Заполняет поле slug для уже существующих городов, у которых slug IS NULL."""
    updated_count = 0
    cities_without_slug_res = await session.execute(
        select(City).where((City.slug.is_(None)) | (City.slug == ""))
    )
    cities_without_slug = cities_without_slug_res.scalars().all()
    if not cities_without_slug:
        return 0

    seed_map = {c["name"].strip().lower(): c["slug"].strip().lower() for c in RUSSIA_CITIES}
    for city in cities_without_slug:
        clean_name = city.name.strip().lower()
        matched_slug = seed_map.get(clean_name)
        if not matched_slug:
            for seed_name, seed_slug in seed_map.items():
                if seed_name in clean_name or clean_name in seed_name:
                    matched_slug = seed_slug
                    break
        if matched_slug:
            city.slug = matched_slug
            updated_count += 1
            logger.info(f"Привязан slug '{matched_slug}' для существующего города '{city.name}' (ID: {city.id})")
    if updated_count > 0:
        await commit_or_rollback(session)
        logger.info(f"✅ Обновлено {updated_count} слагов для существующих городов.")
...
# ========== НОВЫЕ ФУНКЦИИ ДЛЯ ПАРСИНГА (устаревшие, но оставлены для совместимости) ==========
async def upsert_station_fuel_price(
    ...
    fuel_type_clean = str(fuel_type).strip().upper()
    fuel_type_enum = MAPPING.get(fuel_type_clean)

    if not fuel_type_enum:
        for ft in FuelType:
            if ft.value.upper() == fuel_type_clean or ft.name.upper() == fuel_type_clean:
                fuel_type_enum = ft
                break
...
# ========== ИСПРАВЛЕННАЯ find_nearest_green_station (с поддержкой fuel_type и фоллбэком) ==========
async def find_nearest_green_station(
    db: AsyncSession,
    city_id: int,
    lat: float,
    lon: float,
    radius_km: float = 5.0,
    fuel_type: FuelType = FuelType.AI_95
) -> Optional[Station]:
    """
    Находит ближайшую АЗС с подтверждённым наличием указанного топлива.
    Сначала ищет свежие отчёты (GREEN за 2 часа), затем станции с актуальными ценами.
    """
    from utils.helpers import haversine_distance

    cutoff = datetime.now(timezone.utc) - timedelta(hours=2)

    # 1. Поиск по AvailabilityReport (свежие GREEN-отчёты)
    subq = (
    ...
    # 2. Фоллбэк: станция с актуальной ценой в fuel_prices (свежая за 24 часа)
    cutoff_price = datetime.now(timezone.utc) - timedelta(hours=24)
    price_subq = (
        select(FuelPrice.station_id, func.max(FuelPrice.recorded_at).label('max_date'))
        .where(
            FuelPrice.fuel_type == fuel_type,
            FuelPrice.recorded_at >= cutoff_price,
            FuelPrice.is_fresh == True,
            FuelPrice.price.between(30, 150)
        )
    ...
    result = await db.execute(
        select(Station)
        .join(price_subq, Station.id == price_subq.c.station_id)
        .where(Station.city_id == city_id, Station.is_active == True)
    )
    stations = result.scalars().all()
    ...
    if stations:
        nearest = min(stations, key=lambda s: haversine_distance(lat, lon, s.latitude, s.longitude))
        if haversine_distance(lat, lon, nearest.latitude, nearest.longitude) <= radius_km * 2:
            return nearest

    return None

# ========== Bounding Box для геопоиска (с selectinload) ==========
async def get_stations_in_radius(db: AsyncSession, city_id: int, lat: float, lon: float, radius_km: float = 35.0) -> List[Station]:
    """
    Поиск АЗС в радиусе строго внутри указанного города.
    Если в радиусе ничего не найдено, возвращает все активные станции города.
    """
    ...
    # Если в радиусе пусто (например, окраина) — возвращаем все активные станции ЭТОГО города
    if not stations:
        fallback_query = (
            select(Station)
            .where(
...
# ========== ИСПРАВЛЕННАЯ set_user_timezone (с проверкой наличия библиотек) ==========
...
# ========== ПАКЕТНОЕ ОБНОВЛЕНИЕ АДРЕСОВ ==========
async def fill_missing_station_addresses(session: AsyncSession, limit: int = 100) -> Tuple[int, int]:
    """
    Находит АЗС с пустыми адресами (NULL, пустая строка или 'Адрес уточняется'),
    у которых есть валидные координаты, и проставляет точные адреса.
    Возвращает (обновлено_успешно, всего_найдено).
    """
    stmt = (
        select(Station)
        .where(
            ...
            or_(
                ...
                Station.address == "",
                Station.address.ilike("%уточн%"),
                Station.address.ilike("%не указан%")
            )
        )
        .limit(limit)
    )
    ...
    for st in stations:
        try:
            address = await reverse_geocode(st.latitude, st.longitude, session=session)
            if address:
                st.address = address
                updated += 1
                logger.info(f"Обновлен адрес АЗС ID {st.id} ({st.brand}): {address}")
            await asyncio.sleep(1.1)
        except Exception as e:
            logger.error(f"Ошибка при определении адреса АЗС ID {st.id}: {e}")
            continue

    if updated > 0:
        await commit_or_rollback(session)
    return updated, total_found

# ========== СИНХРОНИЗАЦИЯ СТАНЦИЙ ИЗ 2ГИС ==========
async def sync_dgis_city_stations(
    session: AsyncSession,
    city_id: int,
    ...
    """
    Атомарная синхронизация станций из 2ГИС:
    - Создает новые станции с точными координатами и адресами
    - Обновляет адреса у существующих станций без дублирования
    - Добавляет стартовые котировки топлива
    Возвращает (добавлено_станций, обновлено_адресов)
    """
    now = datetime.now(timezone.utc)
    stations_added = 0
    stations_updated = 0
    ...
    for item in stations_data:
        ...
        for fuel_code, price_val in default_prices.items():
            fuel_type_enum = FuelType(fuel_code)
            p_stmt = select(FuelPrice).where(
                FuelPrice.station_id == station.id,
                FuelPrice.fuel_type == fuel_type_enum,
                FuelPrice.is_fresh == True
            )
            existing_p = (await session.execute(p_stmt)).scalar_one_or_none()
            if not existing_p:
...
# ========== УМНЫЙ ПОИСК ГОРОДА С ГЕОКОДИРОВАНИЕМ ==========

# Популярные синонимы и сокращения
CITY_ALIASES = {
...
async def find_or_create_city_by_query(db: AsyncSession, query_text: str) -> Optional[City]:
    """
    Ищет город в базе по названию/алиасу, а если нет — геокодирует координаты и добавляет в БД.
    """
    clean_query = query_text.strip().lower()
    # Удаляем префиксы "г.", "город", "поселок", "п.", "с."
    prefixes = ["город ", "г. ", "г.", "поселок ", "п. ", "с. "]
    for prefix in prefixes:
        if clean_query.startswith(prefix):
            clean_query = clean_query[len(prefix):].strip()
            break

    canonical_name = CITY_ALIASES.get(clean_query, clean_query)

    # 1. Поиск точного или частичного совпадения в локальной БД городов
    stmt = select(City).where(
        or_(
            City.name.ilike(f"{canonical_name}"),
            City.name.ilike(f"%{canonical_name}%")
        )
    ).order_by(func.length(City.name).asc()).limit(1)
    result = await db.execute(stmt)
    city = result.scalar_one_or_none()
    if city:
        return city

    # 2. Если города ещё нет в БД — геокодируем его через OpenStreetMap / Nominatim
    from utils.geocoder import geocode_address
    coords = await geocode_address(f"город {canonical_name}")
    if not coords:
        coords = await geocode_address(canonical_name)
