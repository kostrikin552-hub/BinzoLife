# database/crud.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (с геокодированием адресов)
import random
import string
import math
import logging
...
from utils.geocoder import reverse_geocode  # <-- импорт для геокодинга
...
async def fill_missing_station_addresses(session: AsyncSession, limit: int = 100) -> Tuple[int, int]:
    ...
    """
    # Выбираем станции без адреса, но с ненулевыми координатами
    stmt = (
        select(Station)
        .where(
            Station.is_active == True,
    ...
    for st in stations:
        try:
            ...
            # Вежливая задержка между запросами к геокодеру (1 сек по правилам Nominatim)
