# database/crud.py – ПОЛНАЯ ВЕРСИЯ с функциями для бесплатных поисков
...
# ========== НОВЫЕ ФУНКЦИИ ДЛЯ БЕСПЛАТНЫХ ПОИСКОВ ==========
async def can_use_free_search(db: AsyncSession, user_id: int) -> bool:
    """
    Проверяет, может ли пользователь выполнить бесплатный поиск сегодня.
    Если сегодня ещё не использовал – возвращает True (и сбрасывает лимит до 1).
    Если использовал – возвращает False.
    """
    user = await get_user_by_id(db, user_id)
    if not user:
        return False
    ...
    if user.last_free_search_date != today:
        # Сбрасываем лимит на сегодня (1 поиск)
...
async def use_free_search(db: AsyncSession, user_id: int) -> int:
    """
    Уменьшает счётчик бесплатных поисков на 1.
    Возвращает оставшееся количество на сегодня (0 или 1).
    """
    user = await get_user_by_id(db, user_id)
    if not user:
        return 0
