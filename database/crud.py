# database/crud.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (исправлена ошибка MultipleResultsFound)
import random
import string
import math
import logging
...
# ========== ИМПОРТ ГОРОДОВ (ИСПРАВЛЕН) ==========
async def seed_all_russian_cities(session: AsyncSession) -> int:
    """Импортирует недостающие города из RUSSIA_CITIES (без дублирования)."""
    added_count = 0
    existing_res = await session.execute(select(City.slug, City.name))
    rows = existing_res.all()
    existing_slugs = {r[0] for r in rows if r[0]}
    existing_names = {r[1].lower().strip() for r in rows if r[1]}

    for city_data in RUSSIA_CITIES:
        slug = city_data["slug"].strip().lower()
        name_clean = city_data["name"].strip().lower()
        if slug not in existing_slugs and name_clean not in existing_names:
            new_city = City(
    ...
    if added_count > 0:
        ...
        logger.info(f"✅ Успешно добавлено {added_count} новых городов в базу данных.")
    return added_count

# ========== ОБНОВЛЕНИЕ СЛАГОВ (ИСПРАВЛЕН) ==========
...
async def upsert_station_fuel_price(
    ...
    # Исправленный запрос: выбираем самую свежую запись для данной пары (station_id, fuel_type)
    stmt = (
        select(FuelPrice)
        .where(
            and_(
