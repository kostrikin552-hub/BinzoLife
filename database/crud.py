# database/crud.py – ПОЛНЫЙ ИСПРАВЛЕННЫЙ ФАЙЛ
...
async def activate_trial(db: AsyncSession, user_id: int) -> bool:
    """
    Активирует 3-дневный пробный период PRO.
    ВСЕГДА устанавливает pro_until = now + 3 дня, независимо от предыдущего состояния.
    """
    user = await get_user_by_id(db, user_id)
