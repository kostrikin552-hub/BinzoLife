# database/crud.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (с умным поиском города)
import random
import string
import math
import logging
...
async def get_stations_in_radius(db: AsyncSession, city_id: int, lat: float, lon: float, radius_km: float = 35.0) -> List[Station]:
    ...
    query = (
        select(Station)
        .where(
            Station.city_id == city_id,  # СТРОГАЯ ПРИВЯЗКА К ГОРОДУ!
            Station.is_active == True,
...
async def sync_dgis_city_stations(
    ...
    # Индекс станций по координатам с точностью ~100м
    coord_index = {
    ...
    for item in stations_data:
        ...
        if not station:
            ...
            # Если адрес был пустым или неточным — дополняем его идеальным из 2ГИС
        ...
        # Гарантируем наличие базовых котировок топлива на станции
...
async def find_or_create_city_by_query(db: AsyncSession, query_text: str) -> Optional[City]:
    ...
    canonical_name = CITY_ALIASES.get(clean_query, query_text.strip())
    ...
    if coords:
        lat, lon = coords
        # Создаём новый город с реальными координатами
        city = City(
