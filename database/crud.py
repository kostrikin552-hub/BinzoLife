...
# ========== НОВЫЕ ФУНКЦИИ (триал, история, сегментация, тишина) ==========
...
async def get_top_cities(db: AsyncSession, limit: int = 5) -> List[dict]:
    """Топ-N городов по числу пользователей"""
    result = await db.execute(
        select(
            City.name,
...
async def get_feature_usage(db: AsyncSession, days: int = 7) -> dict:
    """Использование основных фич за последние N дней"""
    cutoff = datetime.now(timezone.utc) - timedelta(days=days)
    ...
    for action in actions:
        count = await db.execute(
            ...
            .where(
                ...
                UserAction.recorded_at >= cutoff
            )
        )
        usage[action] = count.scalar() or 0
    # Просмотр профиля – отдельно (пока нет явного действия, можно считать по количеству вызовов команды)
    # Для простоты оставим заглушку
    usage["profile_view"] = 0  # можно добавить логирование позже
    return usage

async def get_pro_retention_stats(db: AsyncSession) -> dict:
    """Статистика по PRO: продления, отписки, churn"""
    now = datetime.now(timezone.utc)
    month_ago = now - timedelta(days=30)

    # Всего пользователей, у которых была PRO в последние 30 дней
    ...
    # Churn rate: кто был PRO месяц назад, но сейчас не активен
...
async def get_conversion_funnel(db: AsyncSession) -> dict:
    """Конверсия между стадиями воронки (в процентах)"""
    funnel = await get_funnel_stats(db)  # возвращает словарь {stage: count}
    total_first_search = funnel.get(1, 0)  # стадия 1 — первый поиск
    ...
    conversions = []
    # от 0 к 1 (не все доходят до поиска)
    if result[0] > 0:
...
async def get_marketing_stats(db: AsyncSession) -> dict:
    ...
    # LTV (оценка: ARPU * средняя длительность подписки в месяцах)
    # Пока возьмём 2 месяца как среднее (можно позже вычислить точнее)
