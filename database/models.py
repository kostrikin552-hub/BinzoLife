# database/models.py — ПОЛНАЯ ВЕРСИЯ (все изменения)
...
class FuelPrice(Base):
    ...
    updated_by_user_id = Column(BigInteger, nullable=True)  # для краудсорсинга
...
class User(Base):
    ...
    trial_alert_sent = Column(Boolean, default=False)   # для этапа 5
    invited_count = Column(Integer, default=0)          # для этапа 5
    ...
    emergency_access_until = Column(DateTime(timezone=True), nullable=True)  # этап 2
...
class UserAchievement(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    achievement_code = Column(String(64), nullable=False)  # вместо achievement_type
...
class ProNotificationSent(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    notification_type = Column(String(20), nullable=False)  # '3d', '2d', '1d', '3h', '1h', 'onboarding', 'trial_expiring'
