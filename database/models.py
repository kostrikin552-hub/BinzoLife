...
class User(Base):
    ...
    timezone_offset = Column(Integer, nullable=True)  # смещение в минутах от UTC
    last_lat = Column(Float, nullable=True)          # последняя широта
    last_lon = Column(Float, nullable=True)          # последняя долгота
