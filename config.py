...
class Settings:
    def __init__(self):
        self.BOT_TOKEN: str = (
            ...
            os.getenv("TG_TOKEN") or os.getenv("TOKEN") or ""
        )
        ...
        raw_db_url = (
            os.getenv("DATABASE_URL") or os.getenv("DB_URL") or
        ...
        # ===== ИСПРАВЛЕННЫЙ ПАРСИНГ ADMIN_IDS =====
        raw_admin_ids = os.getenv("ADMIN_IDS") or os.getenv("ADMINS") or os.getenv("ADMIN_ID") or ""
        self.ADMIN_IDS: List[int] = []
        if raw_admin_ids:
            # Удаляем скобки, кавычки, пробелы
            cleaned = re.sub(r'[\[\]\(\)\'\"]', '', raw_admin_ids)
        ...
        self.PAYMENT_PROVIDER_TOKEN: str = (
            os.getenv("PAYMENT_PROVIDER_TOKEN") or os.getenv("PAYMENTS_TOKEN") or
            os.getenv("PROVIDER_TOKEN") or ""
        )
        ...
        self.YANDEX_GEOCODER_API_KEY: str = os.getenv("YANDEX_GEOCODER_API_KEY", "")
