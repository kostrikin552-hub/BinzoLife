# main.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (с healthcheck, индексами, миграциями и всеми исправлениями)
import os
import asyncio
import logging
import sys
...
async def init_database():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            # Индекс для ускорения поиска АЗС по городу и координатам
            await db.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_stations_city_coords 
                ON stations (city_id, is_active, latitude, longitude);
            """))

            # Индекс для fuel_prices по station_id и fuel_type
            await db.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_fuel_prices_station_fuel 
                ON fuel_prices (station_id, fuel_type, recorded_at DESC);
            """))

            # Индекс для availability_reports
            await db.execute(text("""
                CREATE INDEX IF NOT EXISTS idx_availability_station_fuel_fresh 
                ON availability_reports (station_id, fuel_type, recorded_at DESC) 
                WHERE is_fresh = true;
            """))
