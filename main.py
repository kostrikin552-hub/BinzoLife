...
async def ensure_schema_updates():
    ...
    # НОВЫЕ КОЛОНКИ ДЛЯ БЕСПЛАТНЫХ ПОИСКОВ
        ...
        try:
            ...
            if "RetryAfter" in str(e) or "flood" in str(e).lower():
                ...
                if match:
                    ...
                    wait = 5 * (attempt + 1)
                logger.warning(f"Flood control при отправке админу, ждём {wait} секунд...")
                await asyncio.sleep(wait)
            else:
                logger.error(f"Ошибка отправки сообщения админу: {e}")
                return
    logger.error(f"Не удалось отправить сообщение админу после {max_retries} попыток")
...
async def main():
    ...
    # Если вышли из цикла (например, polling остановился) — завершаем
