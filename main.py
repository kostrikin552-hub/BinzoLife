# main.py — ПОЛНАЯ ВЕРСИЯ БЕЗ СПАМ-РАССЫЛОК
import os
import asyncio
import logging
import sys
...
# Фоновые сервисы (оставляем нужные, спам-воркеры не запускаем)
...
# from services.notifications import price_alert_worker        # <-- СПАМ (отключён)
# from services.pro_notifications import pro_reminder_worker   # <-- СПАМ (отключён)
...
async def cron_alerts_handler(request):
    ...
    # Оставлен для совместимости, но ничего не делает (закомментировано)
    # bot: Bot = request.app["bot"]
    # asyncio.create_task(price_alert_worker(bot))
    return web.Response(text="Success: Alerts disabled", status=200)
...
async def cron_subscriptions_handler(request):
    ...
    return web.Response(text="Success: Subscriptions check disabled", status=200)
...
async def main():
    ...
    # Фоновые задачи (СПАМ-ВОРКЕРЫ УБРАНЫ)
    background_tasks: List[asyncio.Task] = [
        ...
        # asyncio.create_task(run_supervised(lambda: price_alert_worker(bot), "PriceAlerts")),     # УБРАН
        # asyncio.create_task(run_supervised(lambda: pro_reminder_worker(bot), "ProReminders")),   # УБРАН
