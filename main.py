# main.py – ПОЛНЫЙ ИСПРАВЛЕННЫЙ ФАЙЛ (конфликты решаются через паузу, без пересоздания бота)
...
# Подавляем надоедливые предупреждения
...
# ---------- Запуск с повторными попытками (исправлен) ----------
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "Conflict" in error_str or "terminated by other getUpdates" in error_str:
                ...
                # Просто ждём и пробуем снова (не пересоздаём бота)
