...
async def on_startup():
    ...
    # Удаляем вебхук, если был установлен (чтобы не мешал polling)
    try:
    ...
    try:
        await bot.send_message(settings.ADMIN_ID, "✅ Бот успешно запущен и готов к работе (polling)!")
    except Exception as e:
...
async def main():
    ...
    # Запускаем HTTP-сервер для health и задач
