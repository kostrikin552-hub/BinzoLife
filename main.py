...
from handlers import (
    ...
    review, emergency, contest, inline
)
...
from services.radar import send_friday_fuel_radar
from services.data_collector import data_collector_worker  # <-- НОВЫЙ ИМПОРТ
...
async def ensure_schema_updates():
    ...
    # ===== НОВЫЕ ТАБЛИЦЫ ДЛЯ РАДАРА =====
    await create_table_if_not_exists("station_current_fuel", """
    ...
    """)
    await create_table_if_not_exists("station_external_ids", """
    ...
    async with AsyncSessionLocal() as db:
        ...
        indexes = [
            "CREATE INDEX IF NOT EXISTS ix_stations_city_id ON stations(city_id)",
            "CREATE INDEX IF NOT EXISTS ix_stations_lat_lon ON stations(latitude, longitude)",
            "CREATE INDEX IF NOT EXISTS idx_prices_station_fuel_date ON fuel_prices(station_id, fuel_type, recorded_at)",
            "CREATE INDEX IF NOT EXISTS idx_prices_station_id ON fuel_prices(station_id)",
            "CREATE INDEX IF NOT EXISTS idx_prices_fuel_type ON fuel_prices(fuel_type)",
            "CREATE INDEX IF NOT EXISTS ix_availability_station_id ON availability_reports(station_id)",
            "CREATE INDEX IF NOT EXISTS ix_availability_user_id ON availability_reports(user_id)",
            "CREATE INDEX IF NOT EXISTS ix_availability_station_fuel ON availability_reports(station_id, fuel_type)",
            "CREATE INDEX IF NOT EXISTS ix_users_telegram_id ON users(telegram_id)",
            "CREATE INDEX IF NOT EXISTS ix_users_city_id ON users(city_id)",
            "CREATE INDEX IF NOT EXISTS ix_users_is_pro ON users(is_pro)",
            "CREATE INDEX IF NOT EXISTS ix_notifications_user_id ON notifications(user_id)",
            "CREATE INDEX IF NOT EXISTS ix_notifications_station_id ON notifications(station_id)",
            "CREATE INDEX IF NOT EXISTS ix_notifications_active ON notifications(active)",
            "CREATE INDEX IF NOT EXISTS ix_user_actions_user_id ON user_actions(user_id)",
            "CREATE INDEX IF NOT EXISTS ix_user_actions_action ON user_actions(action)",
            "CREATE INDEX IF NOT EXISTS ix_user_actions_recorded_at ON user_actions(recorded_at)",
            "CREATE UNIQUE INDEX IF NOT EXISTS ix_payments_telegram_charge_id ON payments(telegram_payment_charge_id)",
            ...
            "CREATE INDEX IF NOT EXISTS ix_user_economies_user_id ON user_economies(user_id)",
            "CREATE INDEX IF NOT EXISTS ix_user_economies_station_id ON user_economies(station_id)",
            "CREATE INDEX IF NOT EXISTS ix_user_economies_recorded_at ON user_economies(recorded_at)",
            "CREATE INDEX IF NOT EXISTS ix_geocode_cache_lat_lng ON geocode_cache(lat, lng)",
            "CREATE INDEX IF NOT EXISTS ix_pro_notifications_sent_user_id ON pro_notifications_sent(user_id)",
            # НОВЫЕ ИНДЕКСЫ ДЛЯ РАДАРА
        ...
        for sql in indexes:
            try:
                ...
                logger.debug(f"Индекс создан: {sql.split('ON')[1].strip()}")
            except Exception as e:
                logger.warning(f"Не удалось создать индекс: {e}")
        await db.commit()
        logger.info("Все индексы проверены/созданы")
...
async def seed_initial_data():
    """Создаёт город Красноярск, если его нет. Без тестовых станций."""
    async with AsyncSessionLocal() as db:
        city = await db.execute(text("SELECT id FROM cities WHERE name = 'Красноярск'"))
        if not city.fetchone():
            new_city = City(
                name="Красноярск",
            ...
            db.add(new_city)
            await db.commit()
            logger.info("Город Красноярск создан (без тестовых АЗС). Используйте /import_city для загрузки данных.")
        else:
            logger.info("Город Красноярск уже существует, пропускаем создание.")
...
async def on_startup():
    ...
    asyncio.create_task(supervised_task(send_friday_fuel_radar, "friday_radar", restart_delay=3600))
    asyncio.create_task(supervised_task(data_collector_worker, "data_collector", restart_delay=60))  # <-- НОВАЯ ЗАДАЧА
