...
# from services.radar import send_friday_fuel_radar          # временно отключено
# from services.data_collector import data_collector_worker # временно отключено
...
# dp.include_router(inline.router)   # УДАЛЕНО
...
# ---------- Функция обновления схемы БД (без таблиц для радара) ----------
async def ensure_schema_updates():
    ...
    async with AsyncSessionLocal() as db:
        ...
        indexes = [
            ...
            # Удалены индексы для station_current_fuel и station_external_ids
        ]
        for sql in indexes:
            try:
...
# --- Фоновые задачи (без data_collector) ---
...
async def seed_initial_data():
    async with AsyncSessionLocal() as db:
        ...
        if not city.fetchone():
            ...
            db.add(new_city)
            await db.commit()
            logger.info("Город Красноярск создан (без тестовых АЗС).")
        else:
            logger.info("Город Красноярск уже существует")
...
async def on_startup():
    ...
    # Запуск только проверенных фоновых задач
    ...
    # data_collector и radart отключены

    logger.info("Бот запущен с минимальным набором задач (без data_collector и inline)")
