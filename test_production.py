...
# ========== НОВАЯ ФУНКЦИЯ: проверка реальных платежей ==========
async def check_real_payment() -> bool:
    ...
    try:
        ...
        # Проверяем, есть ли PROVIDER_TOKEN
        ...
        prices = [LabeledPrice(label="Тестовый платёж (1 рубль)", amount=100)]  # 1 рубль = 100 копеек
        ...
        error_msg = str(e)
        if "PROVIDER_TOKEN" in error_msg or "Invalid provider token" in error_msg:
            print_check("Реальные платежи", False, f"PROVIDER_TOKEN невалидный: {error_msg[:100]}")
        else:
...
async def run_all_checks():
    logger.info("🚀 Запуск производственного теста BinzoLife (полная проверка)...")
    results = []
    warnings = []
    ...
    # Рекомендации по ручным проверкам
    ...
    if passed == total:
        ...
        logger.info("🚀 Выполните ручные проверки и запускайте рекламу!")
    else:
