# handlers/start.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (с обновлением координат)
import html
import logging
...
async def city_select(callback: types.CallbackQuery):
    """Выбор города по названию (из популярных) с обновлением координат."""
...
async def select_city_by_id(callback: types.CallbackQuery):
    """Выбор города по ID (из пагинированного списка) с обновлением координат."""
