...
from database.crud import (
    get_user, get_city_by_id, find_nearest_green_station, get_latest_fresh_price,
    save_user_location, has_emergency_access, get_recent_emergency_payment,
    commit_or_rollback
)
...
EMERGENCY_PAYWALL_TEXT = (
    "⛽ <b>Ближайшая колонка с топливом найдена!</b>\n\n"
    "Для разового экстренного доступа активируйте поиск за <b>50 ₽</b> (или 50 ⭐️ Stars).\n"
    "<i>Доступ будет активен 2 часа без ограничений.</i>"
)
...
async def check_availability_and_offer(message: types.Message, state: FSMContext, lat: float, lon: float):
    async with AsyncSessionLocal() as db:
        ...
        user_fuel = user.default_fuel if user and user.default_fuel else FuelType.AI_95

        station = await find_nearest_green_station(
            db, city.id, lat, lon, radius_km=5.0, fuel_type=user_fuel
        )

        if not station and has_access:
            station = await find_nearest_green_station(
                db, city.id, lat, lon, radius_km=10.0, fuel_type=user_fuel
            )

        if not station:
            preview_station = await find_nearest_green_station(
                db, city.id, lat, lon, radius_km=20.0, fuel_type=user_fuel
            )
            if not preview_station:
                await message.answer(
                    "❌ В радиусе 20 км нет АЗС с подтверждённым наличием топлива.\n"
            ...
            if has_access:
                station = preview_station
            else:
                await state.set_state(EmergencyStates.waiting_payment)
                await message.answer(
                    f"🔍 Проверяю данные…\n\n"
                    f"✅ Рядом с вами есть АЗС с топливом. Ближайшая — в {dist:.1f} км.\n\n"
...
async def pay_emergency_rub(callback: types.CallbackQuery):
    ...
    await send_invoice(
        ...
        amount=5000,
        payload=order_id,
        description="Экстренный поиск АЗС",
        currency="RUB",
        need_email=True
    )
...
async def show_result(message: types.Message, station, lat: float, lon: float):
    async with AsyncSessionLocal() as db:
        user_fuel = FuelType.AI_95
        price = await get_latest_fresh_price(db, station.id, user_fuel)
        ...
        await message.answer(
            ...
            f"\n🗺 <a href='{nav_url}'>Открыть маршрут</a>\n\n"
