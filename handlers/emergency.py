# handlers/emergency.py — ПОЛНАЯ ВЕРСИЯ (все изменения)
import logging
import html
...
async def check_availability_and_offer(message: types.Message, state: FSMContext, lat: float, lon: float):
    async with AsyncSessionLocal() as db:
        ...
        # Проверяем доступ к SOS (PRO или оплата за последние 2 часа)
        ...
        if not station:
            if has_access:
                # Расширяем радиус до 10 км для PRO/оплативших
                ...
                # Не PRO и нет оплаты — предложить оплатить
                dist = haversine_distance(lat, lon, station.latitude, station.longitude) if station else 0
                await state.set_state(EmergencyStates.waiting_payment)
                await message.answer(
                    f"🔍 Проверяю данные…\n\n"
                    f"✅ Рядом с вами есть АЗС с АИ‑95. Ближайшая — в {dist:.1f} км.\n\n"
