# handlers/admin.py — ИСПРАВЛЕННАЯ ВЕРСИЯ (для нового парсера)
import csv
import io
import re
import asyncio
...
from database.crud import (
    ...
    commit_or_rollback, update_city_prices_by_brand
)
...
# ---------- ПРИНУДИТЕЛЬНОЕ ОБНОВЛЕНИЕ ДАННЫХ (НОВАЯ ВЕРСИЯ) ----------
...
async def refresh_city_cmd(message: types.Message):
    ...
    async with AsyncSessionLocal() as session:
        # Находим город по слагу или названию
        ...
        prices = await fuel_parser.fetch_fuelprices_city(city.slug)
        if prices:
            for item in prices:
...
async def refresh_all_cities_cmd(message: types.Message):
    ...
    async with AsyncSessionLocal() as session:
        for city in cities:
            ...
            await asyncio.sleep(0.5)  # небольшая пауза между городами
