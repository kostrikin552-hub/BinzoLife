...
# ==================== ИСПРАВЛЕННАЯ КОМАНДА /import_all_cities ====================
...
async def import_all_cities_cmd(message: types.Message):
    ...
    for idx, url in enumerate(city_urls, 1):
        ...
        # Пауза между городами, чтобы не перегружать БД
        await asyncio.sleep(2)

    # Отправляем отчёт частями, чтобы не превысить лимит сообщений
    report_lines = [
    ...
    # Отправляем по частям (Telegram ограничение 4096 символов)
...
async def clean_test_data_cmd(message: types.Message):
    ...
    async with AsyncSessionLocal() as db:
        async with db.begin():
            ...
            # 2. Удаляем отзывы от администратора
