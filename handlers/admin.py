# handlers/admin.py – ПОЛНЫЙ (с исправленной командой /selftest, fallback на parse_mode=None)
...
# ---------- КОМАНДА SELF TEST (исправленная, с fallback) ----------
        ...
        try:
            if len(summary) > 4000:
                for i in range(0, len(summary), 4000):
                    await message.answer(summary[i:i+4000], parse_mode="HTML")
            else:
                await message.answer(summary, parse_mode="HTML")
        except Exception as e:
            # Если HTML не проходит, отправляем без форматирования
            logger.warning(f"Ошибка HTML-форматирования: {e}, отправляем без форматирования")
            if len(summary) > 4000:
                for i in range(0, len(summary), 4000):
