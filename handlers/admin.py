...
async def show_stats(message: types.Message):
    ...
    # Топ городов
    text += "📍 **Топ-5 городов по пользователям**\n"
    ...
    # Воронка
    text += "🔄 **Воронка (от первого поиска)**\n"
    ...
    # Отзывы
    text += f"⭐ **Отзывы**: {review['total_reviews']}, ср. рейтинг {review['avg_rating']}⭐\n\n"

    # Рефералы
    text += f"👥 **Рефералы**: всего {referral['total_referrals']}, получили бонус {referral['rewarded']} ({round(referral['rewarded']/referral['total_referrals']*100, 1) if referral['total_referrals'] else 0}%)\n\n"

    # Использование фич
    text += "⚙️ **Популярные фичи (за 7 дней)**\n"
    ...
    # Ошибки (заглушка)
    text += "\n⚠️ **Ошибки (за 7 дней)**: пока не собираются (добавьте логирование)\n"
