# handlers/admin.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ
import csv
import io
import re
import asyncio
...
# ---------- Импорт всех городов (улучшенная версия) ----------
...
async def import_all_cities_cmd(message: types.Message):
    ...
    await message.answer(f"🔄 Начинаю импорт всех {len(city_urls)} городов... Это может занять до 10 минут.", parse_mode=None)

    results = []
    total_success = 0
    total_fail = 0

    for idx, url in enumerate(city_urls, 1):
        try:
            logger.info(f"Импорт {idx}/{len(city_urls)}: {url}")
            res = await import_city_from_url(url)
            if "error" in res:
                results.append(f"❌ {idx}. {url} — ошибка: {res['error']}")
                total_fail += 1
            else:
                results.append(
                    f"✅ {idx}. {res['city']} — "
            ...
            logger.error(f"Исключение при импорте {url}: {e}")
            results.append(f"❌ {idx}. {url} — критическая ошибка: {e}")
            total_fail += 1

        await asyncio.sleep(2)

    report_lines = [
        f"📊 Итоги импорта всех городов:",
...
# ---------- Экспорт пользователей (пагинация) ----------
...
async def show_stats(message: types.Message):
    ...
    text_stat = "📊 <b>Статистика BinzoLife</b>\n\n"

    text_stat += "👥 <b>Пользователи</b>\n"
    text_stat += f"▪ Всего: {user['total_users']}\n"
    text_stat += f"▪ Активных за 7 дней: {user['active_users_7d']} ({round(user['active_users_7d']/user['total_users']*100, 1) if user['total_users'] else 0}%)\n"
    text_stat += f"▪ Сделали поиск: {user['have_searches']} ({round(user['have_searches']/user['total_users']*100, 1) if user['total_users'] else 0}%)\n"
    text_stat += f"▪ Активных PRO: {user['active_pro']}\n"
    text_stat += f"▪ Новых сегодня: {user['new_today']}\n"
    text_stat += f"▪ За неделю: {user['new_week']}\n"
    text_stat += f"▪ За месяц: {user['new_month']}\n\n"

    text_stat += "📍 <b>Топ-5 городов по пользователям</b>\n"
    for i, city in enumerate(top_cities, 1):
        text_stat += f"{i}. {city['city']} — {city['users']}\n"
    text_stat += "\n"

    text_stat += "💳 <b>Платежи</b>\n"
    text_stat += f"▪ Всего оплат: {payment['total_payments']}\n"
    text_stat += f"▪ Выручка: {payment['total_revenue']:.2f} ₽\n"
    text_stat += f"▪ ARPU (PRO): {arpu:.2f} ₽\n"
    text_stat += f"▪ Конверсия поиск → PRO: {conversion}%\n"
    text_stat += f"▪ Средний LTV (оценка): {ltv:.2f} ₽\n"
    text_stat += f"▪ Сегодня: {payment['payments_today']} шт. ({payment['revenue_today']:.2f} ₽)\n"
    text_stat += f"▪ За неделю: {payment['payments_week']} шт. ({payment['revenue_week']:.2f} ₽)\n"
    text_stat += f"▪ За месяц: {payment['payments_month']} шт. ({payment['revenue_month']:.2f} ₽)\n\n"

    text_stat += "👑 <b>PRO-пользователи</b>\n"
    text_stat += f"▪ Активных: {pro['active_pro']}\n"
    text_stat += f"▪ Продлили подписку: {pro['renewed']} ({round(pro['renewed']/pro['active_pro']*100, 1) if pro['active_pro'] else 0}% от активных)\n"
    text_stat += f"▪ Отписалось за месяц: {pro['churned']} (churn {pro['churn_rate']}%)\n"
    text_stat += f"▪ PRO по рефералам (оценка): {referral['rewarded']} ({round(referral['rewarded']/pro['active_pro']*100, 1) if pro['active_pro'] else 0}%)\n\n"

    text_stat += "🔄 <b>Воронка (от первого поиска)</b>\n"
    ...
    for i, stage in enumerate([0,1,2,3,4,5]):
        ...
        if i == 0:
            text_stat += f"▪ {name}: {count} (100%)\n"
        else:
            conv = convs[i-1] if i-1 < len(convs) else 0
            text_stat += f"▪ {name}: {count} ({conv}% от предыдущего этапа)\n"
    if len(convs) > 1:
        drop = round(100 - convs[1], 1) if convs[1] is not None else 0
        text_stat += f"⚠️ Самый сильный отвал — между 1 и 3 днём ({drop}%)\n\n"

    text_stat += f"⭐ <b>Отзывы</b>: {review['total_reviews']}, ср. рейтинг {review['avg_rating']}⭐\n\n"

    text_stat += f"👥 <b>Рефералы</b>: всего {referral['total_referrals']}, получили бонус {referral['rewarded']} ({round(referral['rewarded']/referral['total_referrals']*100, 1) if referral['total_referrals'] else 0}%)\n\n"

    text_stat += "⚙️ <b>Популярные фичи (за 7 дней)</b>\n"
    total_actions = sum(feature.values())
    if total_actions:
        for action, count in feature.items():
            percent = round(count / total_actions * 100, 1)
            text_stat += f"▪ {action.replace('_', ' ').title()}: {count} ({percent}%)\n"
    else:
        text_stat += "▪ Нет данных\n"

    text_stat += "\n⚠️ <b>Ошибки (за 7 дней)</b>: пока не собираются (добавьте логирование)\n"

    if len(text_stat) > 4000:
        for i in range(0, len(text_stat), 4000):
            await message.answer(text_stat[i:i+4000], parse_mode="HTML")
    else:
        await message.answer(text_stat, parse_mode="HTML")
...
async def cities_stats_cmd(message: types.Message):
    async with AsyncSessionLocal() as db:
        ...
        text_stat = "🏙 <b>Статистика АЗС по городам</b>\n\n"
        for city in stats:
        ...
        text_stat += f"📊 <b>Итого:</b> активных АЗС: {total_active}, всего: {total_all}"
        await message.answer(text_stat, parse_mode="HTML")
...
async def stations_without_address_cmd(message: types.Message):
    async with AsyncSessionLocal() as db:
        ...
        if count == 0:
            ...
            return
        text_msg = f"🛢 Найдено {count} станций без адреса:\n\n"
        for station in stations[:20]:
            city_name = station.city.name if station.city else "неизвестен"
            text_msg += f"ID: {station.id} — {station.name} (город {city_name})\n"
        if count > 20:
            text_msg += f"... и ещё {count - 20} станций."
