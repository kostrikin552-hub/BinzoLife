...
async def clean_all_addresses_cmd(message: types.Message):
    """Очищает адреса всех АЗС от цен, HTML-тегов и лишней информации"""
...
async def clean_station_names_cmd(message: types.Message):
    """Очищает названия всех АЗС от цен, дат и лишней информации"""
...
async def refresh_city_cmd(message: types.Message):
    """Принудительно обновляет данные для одного города (цены, адреса, координаты)"""
...
async def refresh_all_cities_cmd(message: types.Message):
    """Принудительно обновляет данные для всех городов (цены, адреса, координаты)"""
...
# ====================================================================
# НОВАЯ КОМАНДА: /test_prod – полный производственный тест с детальными сообщениями
# ====================================================================
@router.message(Command("test_prod"))
@admin_only
async def test_production_cmd(message: types.Message):
    """Запускает полный производственный тест и отправляет отчёт с деталями."""
    ...
    try:
        ...
        for name, ok, msg in results:
            icon = "✅" if ok else "❌"
            # Ограничиваем длину сообщения, чтобы не превысить лимит
            if len(msg) > 100:
                msg = msg[:97] + "..."
        ...
        lines.append("• Проверьте, что тестовый инвойс пришёл админу – если да, то PROVIDER_TOKEN работает")
        ...
        lines.append("• Проверьте логи на наличие ошибок в pro_expiry_notifier")
