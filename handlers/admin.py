# handlers/admin.py – ПОЛНЫЙ (с исправленной командой /selftest, без HTML)
...
# ---------- КОМАНДА SELF TEST (БЕЗ HTML, parse_mode=None) ----------
...
async def self_test(message: types.Message):
    ...
    try:
        ...
        # Отправляем без форматирования, чтобы избежать ошибок парсинга HTML
        if len(summary) > 4000:
