...
from sqlalchemy import select, func, delete, or_, text  # <-- ДОБАВЛЕН text
...
# ====================================================================
# НОВАЯ КОМАНДА: /clean_test_data – удаление тестовых данных
# ====================================================================
@router.message(Command("clean_test_data"))
@admin_only
async def clean_test_data_cmd(message: types.Message):
    """Удаляет тестовых пользователей, отзывы, тестовые платежи."""
    await message.answer("🔄 Начинаю очистку тестовых данных... Это может занять несколько секунд.")

    async with AsyncSessionLocal() as db:
        async with db.begin():
            # 1. Находим тестовых пользователей (telegram_id < 0)
            ...
            if test_user_ids:
                ...
                logger.info(f"Удалено {len(test_users)} тестовых пользователей")

            # 2. Удаляем отзывы от администратора (если вы их писали)
            admin_user = await db.execute(
                select(User).where(User.telegram_id == int(settings.ADMIN_ID))  # <-- ИСПРАВЛЕНО
            )
    ...
    await message.answer(
        ...
        f"🧹 Удалено тестовых пользователей: {len(test_users)}\n"
        ...
        "Теперь база данных чиста. Можно запускать рекламу! 🚀"
    )
