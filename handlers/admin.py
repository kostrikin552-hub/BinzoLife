# handlers/admin.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (с 2ГИС и обновлёнными импортами)
import csv
import io
import re
import asyncio
...
from database.crud import (
    ...
    fill_missing_station_addresses, sync_dgis_city_stations
)
...
# ---------- ПРИНУДИТЕЛЬНОЕ ОБНОВЛЕНИЕ ДАННЫХ (устаревшие) ----------
@router.message(Command("refresh_city"))
@admin_only
async def refresh_city_cmd(message: types.Message):
    await message.answer("⚠️ Команда устарела. Используйте /sync_dgis для синхронизации через 2ГИС.")
...
# ========== СИНХРОНИЗАЦИЯ 2ГИС (ручной запуск) ==========
@router.message(Command("sync_dgis"))
@admin_only
async def admin_sync_dgis_cmd(message: types.Message):
    """
    Синхронизация АЗС города через 2ГИС.
    Формат: /sync_dgis [город_или_slug]
    Пример: /sync_dgis kazan
    """
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
        await message.answer(
            "Формат команды: <code>/sync_dgis [город_или_slug]</code>\n"
            "Пример: <code>/sync_dgis kazan</code> или <code>/sync_dgis москва</code>",
            parse_mode="HTML"
        )
        return
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not city or not city.latitude or not city.longitude:
            await message.answer(f"❌ Город '{query}' не найден или у него не указаны координаты.")
            return

        status_msg = await message.answer(
            f"⏳ <b>2ГИС:</b> Сбор АЗС для г. {city.name}...",
            parse_mode="HTML"
        )
        ...
        try:
            ...
            if not stations:
                await status_msg.edit_text(f"⚠️ 2ГИС не вернул станций для г. {city.name}.")
                return

            added, updated = await sync_dgis_city_stations(session, city.id, stations)

            await status_msg.edit_text(
                f"✅ <b>2ГИС: Синхронизация успешна!</b>\n\n"
                f"Город: <b>{city.name}</b>\n"
                f"• Получено из 2ГИС: <b>{len(stations)}</b> АЗС\n"
                f"• Добавлено новых в БД: <b>{added}</b>\n"
                f"• Уточнено адресов: <b>{updated}</b>",
                parse_mode="HTML"
            )
        except Exception as e:
