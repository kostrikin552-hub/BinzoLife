# handlers/admin.py – ПОЛНЫЙ ФАЙЛ (с исправленной командой /selftest)
...
# ---------- КОМАНДА SELF TEST (финальная, с полной очисткой) ----------
@router.message(Command("selftest"))
@admin_only
async def self_test(message: types.Message):
    """Запуск расширенной самопроверки всех систем"""
    await message.answer("🔄 Запускаю полную самопроверку... Это может занять до 60 секунд.", parse_mode=None)
    try:
        result = await run_self_check()
        summary = result.summary()
        # Дополнительная очистка от угловых скобок
        ...
        await message.answer(f"❌ Критическая ошибка: {error_msg}", parse_mode=None)
