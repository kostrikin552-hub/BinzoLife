# handlers/payments.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (с логированием и защитой от ошибок валют)
import json
import logging
...
TARIFFS = {
    "pro_weekend": {
        ...
        "rub_amount": 4900,   # 49.00 ₽ (в копейках)
        "stars_amount": 45,
        "days": 3,
        "badge": "Попробовать"
    },
    "pro_24h": {
        ...
        "rub_amount": 2900,   # 29.00 ₽ (в копейках)
        "stars_amount": 15,
        "days": 1,
    ...
    "pro_1m": {
        ...
        "rub_amount": 9900,   # 99.00 ₽ (в копейках)
        "stars_amount": 50,
        "days": 30,
        "badge": "Выгода 70%"
    },
    "pro_3m": {
        ...
        "rub_amount": 24900,  # 249.00 ₽ (в копейках)
        "stars_amount": 125,
        "days": 90,
...
async def send_invoice(
    ...
    """
    Отправляет инвойс с проверкой суммы и провайдера.
    """
    if currency == "RUB":
        if amount <= 0:
            logger.error(f"Некорректная сумма для рублёвого платежа: {amount}")
            await message.answer("❌ Ошибка: сумма платежа должна быть больше 0.")
            return
        if not settings.PAYMENT_PROVIDER_TOKEN:
            logger.error("PAYMENT_PROVIDER_TOKEN не задан")
            await message.answer("❌ Оплата картой временно недоступна. Используйте Telegram Stars.")
            return
        # Сумма уже в копейках, передаём как есть
        prices = [LabeledPrice(label=description, amount=amount)]
        provider_token = settings.PAYMENT_PROVIDER_TOKEN
    elif currency == "XTR":
        if amount <= 0:
            logger.error(f"Некорректное количество Stars: {amount}")
            await message.answer("❌ Ошибка: количество Stars должно быть больше 0.")
            return
        prices = [LabeledPrice(label=description, amount=amount)]
        provider_token = ""
    else:
        logger.error(f"Неизвестная валюта: {currency}")
        await message.answer("❌ Неизвестная валюта платежа.")
        return

    logger.info(f"Отправка инвойса: currency={currency}, amount={amount}, payload={payload}")
    ...
    try:
        ...
        await message.answer("❌ Не удалось создать платёж. Попробуйте позже или используйте другой способ оплаты.")
...
async def choose_payment_method(callback: types.CallbackQuery):
    ...
    rub_price = t["rub_amount"] // 100  # переводим копейки в рубли для отображения
...
async def send_rub_invoice(callback: types.CallbackQuery):
    ...
    await send_invoice(
        ...
        amount=t["rub_amount"],  # уже в копейках
        payload=payload,
        description=t["title"],
        currency="RUB",
        need_email=True
...
async def pre_checkout_query(pre_checkout: PreCheckoutQuery):
    ...
    if payload.startswith("emergency_rub_"):
        ...
        if len(parts) >= 3 and int(parts[2]) != pre_checkout.from_user.id:
            ...
            return
        if pre_checkout.total_amount != 5000:  # 50 рублей в копейках
    ...
    if payload.startswith("emergency_stars_"):
        ...
        if len(parts) >= 3 and int(parts[2]) != pre_checkout.from_user.id:
            await pre_checkout.answer(ok=False, error_message="Неверный заказ")
            return
        if pre_checkout.total_amount != 50:  # 50 Stars
    ...
    tariff_key = f"{parts[1]}_{parts[2]}" if len(parts) >= 3 else ""
