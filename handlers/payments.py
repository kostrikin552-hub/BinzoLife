# handlers/payments.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (с корректным чеком и fallback)
import json
import logging
...
async def send_invoice(
    ...
    """
    Отправляет инвойс с корректным чеком для российских эквайрингов.
    """
    ...
    if currency == "RUB":
        # Форматируем сумму строго с двумя знаками: "49.00"
                ...
                "items": [
                    {
                        ...
                        "vat_code": 1,  # 1 = без НДС (для УСН/самозанятых)
    ...
    try:
        await message.answer_invoice(
            ...
            description=f"Доступ к функциям BinzoLife: {description}",
        ...
        # Если эквайринг отклонил с чеком, пробуем послать базовый инвойс без чека
        if currency == "RUB" and provider_data:
            try:
                logger.info("Пробуем отправить инвойс без provider_data...")
                await message.answer_invoice(
                    title=f"Оплата: {description[:24]}",
                    description=f"Доступ к функциям BinzoLife: {description}",
                    provider_token=provider_token,
        ...
        await message.answer(
            ...
            "Пожалуйста, воспользуйтесь оплатой через ⭐️ <b>Telegram Stars</b>.",
            parse_mode="HTML"
        )
