# handlers/profile.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (с корректной сменой города)
import html
import logging
...
async def change_city_request(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Удаляем текущее сообщение, чтобы избежать ошибки редактирования с Reply-клавиатурой
    await callback.message.delete()
    await callback.message.answer(
        "🏙 <b>Смена города</b>\n\n"
        "Отправьте геолокацию кнопкой ниже или просто <b>напишите название города</b> в чат (например: <i>Казань</i>, <i>Тюмень</i>, <i>Екб</i>):",
