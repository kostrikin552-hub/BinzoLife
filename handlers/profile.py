# handlers/profile.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ
import html
import logging
...
async def set_fuel(callback: types.CallbackQuery):
    ...
    # После смены топлива возвращаемся в профиль
...
# ---------- Обработка кнопки "Назад" из выбора города (возврат в профиль) ----------
@router.callback_query(F.data == "back_to_profile_from_city")
async def back_to_profile_from_city(callback: types.CallbackQuery):
    await callback.answer()
    await show_profile(callback.message)
