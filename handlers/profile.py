# handlers/profile.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (исправлен user_telegram_id)
import html
import logging
...
from database.crud import (
    ...
    create_user  # добавляем для случая, если пользователь отсутствует
)
...
async def show_profile(message: types.Message, user_telegram_id: int = None):
    """
    Показ профиля. Если вызвано из callback — user_telegram_id обязателен,
    т.к. message.from_user.id в этом случае является ID бота.
    """
    # КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: используем реальный ID пользователя, а не бота
    target_user_id = user_telegram_id if user_telegram_id else message.from_user.id
    logger.info(f"[show_profile] target_user_id={target_user_id}, from_message={message.from_user.id}, from_callback={user_telegram_id}")
...
async def change_city_request(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "🏙 <b>Смена города</b>\n\n"
        "📍 Отправьте геолокацию кнопкой ниже, чтобы определить город автоматически.\n"
        "Или просто <b>напишите название города</b> в чат (например: <i>Казань</i>, <i>Тюмень</i>, <i>Екб</i>):",
...
async def process_location_city(message: types.Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if city:
            ...
            # Сохраняем РЕАЛЬНЫЕ координаты пользователя (не центр города)
            user.last_lat = lat
            user.last_lon = lon
            ...
            await message.answer(
                ...
                f"📍 Ваши точные координаты обновлены.",
...
async def process_text_city(message: types.Message, state: FSMContext):
    ...
    # Игнорируем кнопку, оставляя пользователя в состоянии
    if text_val == "✏️ Написать город текстом":
        await message.answer(
            "✏️ Пожалуйста, введите название вашего города текстом (например: <b>Казань</b>, <b>Тюмень</b>, <b>Екб</b>):",
            parse_mode="HTML"
        )
        return
    ...
    async with AsyncSessionLocal() as db:
        ...
        user.city_id = city.id
        # При текстовом вводе используем координаты центра города (GPS нет)
...
async def change_fuel(callback: types.CallbackQuery):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if user and user.default_fuel:
            default_fuel = user.default_fuel.value if hasattr(user.default_fuel, 'value') else str(user.default_fuel)
        else:
            default_fuel = "АИ-95"
...
async def set_fuel(callback: types.CallbackQuery):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if user:
            try:
                ...
                # Если передан русский код типа "АИ-95" – мапим
                mapping = {"АИ-92": FuelType.AI_92, "АИ-95": FuelType.AI_95,
    ...
    # Передаём реальный ID пользователя из callback
...
async def set_silent(callback: types.CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"✅ Тишина настроена: {start_hour}:00 – {end_hour}:00. Уведомления не будут приходить в этот период."
        )
    except Exception:
        await callback.message.answer(
            f"✅ Тишина настроена: {start_hour}:00 – {end_hour}:00."
        )
...
async def referral_hub(callback: types.CallbackQuery):
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Используем результат get_referral_link, но если он пустой – формируем вручную
        if not ref_link:
