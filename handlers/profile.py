# handlers/profile.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (с обновлением координат при смене города)
import html
import logging
...
# ========== ИСПРАВЛЕННЫЙ ОБРАБОТЧИК ВЫБОРА ГОРОДА (с обновлением координат) ==========
@router.callback_query(lambda c: c.data.startswith("city_select_"))
async def city_select_profile(callback: types.CallbackQuery):
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Обновляем город и координаты пользователя
        ...
        await callback.message.answer(
            f"✅ Город изменён на <b>{html.escape(city.name)}</b>!",
