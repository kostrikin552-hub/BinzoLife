# handlers/find.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (все исправления)
import html
import logging
import io
import asyncio
...
def parse_fuel_type_from_callback(val: str) -> FuelType:
    """Безопасно парсит FuelType из строки callback_data."""
    if not val:
...
def calculate_true_savings(
    ...
    if avg_city_price is None or avg_city_price <= 0 or station_price <= 0:
        return {
            ...
            "net_savings": 0,
            "is_worth": False,
            "badge": "Цены не подтверждены"
        }

    # Если расход равен 0 (электромобиль или ошибка) – считаем только переплату/экономию
    if consumption_per_100km <= 0:
        ...
        return {
            ...
            "trip_cost": 0,
            "net_savings": round(gross_savings, 1),
            "is_worth": gross_savings > 10.0,
            "badge": f"✅ Экономия: +{gross_savings:.0f} ₽ на баке"
        }

    price_diff_per_liter = avg_city_price - station_price

    # Если цена на станции выше средней – переплата
    if price_diff_per_liter < 0:
        overpay = abs(price_diff_per_liter) * tank_volume
        return {
            "gross_savings": 0,
...
# ========== ПОЛУЧЕНИЕ СТАТУСА ТОПЛИВА (с фоллбэком) ==========
async def get_fuel_status(db, station_id: int, fuel_type: FuelType) -> dict:
    """Определяет статус, цену, очередь и источник с каскадным фоллбэком."""
    # 1. Проверяем таблицу оперативного статуса
    ...
    # 2. Фоллбэк: берем из AvailabilityReport и FuelPrice
...
# ========== СИНХРОННАЯ ГЕНЕРАЦИЯ QR-КОДА (выносим в поток) ==========
def _sync_generate_share_image(name: str, price: float, status: str, address: str, ref_link: str) -> bytes:
    """Синхронная генерация изображения для шеринга (вызывается в отдельном потоке)."""
    try:
        ...
        logger.error(f"Ошибка синхронной генерации картинки: {e}")
        return None


async def generate_share_image(name: str, price: float, status: str, address: str, ref_link: str) -> bytes:
    """Асинхронная обёртка для генерации изображения (запускает в отдельном потоке)."""
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(
        None,
        _sync_generate_share_image,
        name,
        price,
        status,
        address,
...
# ===== ЗАЩИТА ОТ ЗАВИСАНИЯ FSM В СОСТОЯНИИ choosing_sort =====
@router.message(FindStates.choosing_sort, F.text)
async def handle_text_in_sort_state(message: types.Message, state: FSMContext):
    """Перехватывает текстовые сообщения в состоянии выбора сортировки и сбрасывает FSM."""
    await state.clear()
    await message.answer(
...
async def handle_any_in_sort_state(message: types.Message, state: FSMContext):
    """Защита от любых других типов сообщений в состоянии выбора сортировки."""
    await state.clear()
    await message.answer(
        "⏳ Выбор сортировки был отменён.",
        reply_markup=main_menu_keyboard()
    )
...
# ---------- ОСНОВНАЯ ФУНКЦИЯ ПОИСКА (ИСПРАВЛЕННАЯ) ----------
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Проверяем координаты: используем GPS только если расстояние от пользователя до центра города < 80 км
            ...
            if not is_pro:
                ...
                if not can_search:
                    await message.answer(
                        ...
                        "🔹 Уведомления о ценах и очередях\n\n"
            ...
            # 1. Загружаем станции СТРОГО выбранного города
            stations = await get_stations_in_radius(db, city.id, search_lat, search_lon, radius_km=35.0)

            # Флаг, что это fallback (если stations == все станции города, а не в радиусе)
            is_fallback = False
            if not stations:
            ...
            for station in stations:
                ...
                # Если у станции нет цены – помечаем её как «без цены» и не ставим в топ
                if not price_rec:
                    results.append({
                        "station": station,
                        "price": None,
                        "price_time": None,
                        "availability": AvailabilityStatus.GRAY,
                        "availability_time": None,
                        "distance_km": haversine_distance(search_lat, search_lon, station.latitude, station.longitude),
                        "rating": 0,
                        "explanation": "Цена не подтверждена",
                        "price_diff": 0,
                        "avg_price": avg_price,
                        "no_price": True,
                    })
                ...
                results.append({
                    ...
                    "availability_time": price_rec.recorded_at,
                    "distance_km": dist,
            ...
            # Сортировка: сначала те, у кого есть цена, потом остальные
            ...
            # Если есть фоллбэк – показываем предупреждение
            if is_fallback:
                warning_msg = "⚠️ В радиусе 35 км заправок не найдено. Показываем ближайшие доступные АЗС в черте города."
...
# ---------- ОТОБРАЖЕНИЕ КАРТОЧКИ (исправлено экранирование и синхронизация статуса) ----------
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext, fuel_type: FuelType = None):
    try:
        ...
        # Извлекаем цену из результата (может быть None)
        ...
        # ========== СТАТУС ТОПЛИВА (с фоллбэком и синхронизацией) ==========
        async with AsyncSessionLocal() as db:
            # Если цена пришла из результата – используем её, иначе запрашиваем заново
        ...
        savings_data = {}
        if not no_price and avg_price and avg_price > 0 and price and price is not None and price > 0 and distance_km > 0:
...
# ---------- ОБРАБОТЧИК "ПОКАЗАТЬ ЕЩЁ 2 ВАРИАНТА" (исправлена логика индексов и кнопок) ----------
...
async def show_more_stations(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not all_results or current_index >= len(all_results):
            await callback.answer("Больше заправок с выбранным топливом в этом радиусе не найдено ⛽", show_alert=True)
            return
        ...
        for i, res in enumerate(more_results, start=next_index+1):
            ...
            text += (
                f"{i}. {station_name} — {price_text}, {status}, {dist_text}\n"
                f"   🗺 {map_link}\n\n"
            )
...
async def back_to_best(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        fuel_type = data.get("fuel_type", FuelType.AI_95)
        if all_results:
            await show_station_card(callback.message, all_results[0], 0, len(all_results), is_pro, state, fuel_type=fuel_type)
        else:
...
# ---------- СООБЩИТЬ ЦЕНУ (передаём fuel_type) ----------
...
async def start_report_price(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"✏️ Введите актуальную цену на этой АЗС для {fuel_code} (в рублях за литр, например, 68.50):",
...
async def process_report_price(message: types.Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if recent_report.scalar_one_or_none():
            await state.clear()
            await message.answer("ℹ️ Вы уже обновляли цену на этой АЗС за последние 24 часа. Спасибо за бдительность!")
            return
    ...
    await message.answer(
        ...
        "Ваш отчёт принят. Благодаря вам десятки водителей района не переплатят за бензин сегодня.\n\n"
        f"⭐️ Вам начислен <b>+1 балл водительской репутации</b>.\n"
        f"Проверить свой прогресс и рейтинг можно в разделе <b>«👤 Профиль»</b>.",
        parse_mode="HTML",
        reply_markup=main_menu_keyboard()
    )
...
async def show_graph(callback: CallbackQuery, state: FSMContext):
    ...
    if not await check_pro(callback.from_user.id):
        await callback.message.answer(
            ...
            "💎 Оформите PRO за 99 ₽/месяц и получите графики!",
            reply_markup=pro_purchase_keyboard(),
            parse_mode="HTML"
        )
        return
