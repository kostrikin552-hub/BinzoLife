# handlers/find.py – с расширенным логированием для отладки карточки
...
# ---------- Функция отображения карточки (с расширенным логированием) ----------
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext):
    try:
        ...
        # Логирование ключевых данных для отладки
        logger.info(f"=== КАРТОЧКА АЗС ===")
        logger.info(f"Station ID: {station.id}")
        logger.info(f"Название: {station.name}")
        logger.info(f"Адрес: {station.address}")
        logger.info(f"Широта: {station.latitude}, Долгота: {station.longitude}")
        logger.info(f"Расстояние (км): {distance_km}")
        logger.info(f"Средняя цена по городу: {avg_price}")
        logger.info(f"Цена АЗС: {price}")
