...
from database.crud import (
    ...
    get_stations_in_radius  # добавлен импорт
)
...
def calculate_true_savings(station_price: float, avg_city_price: float, distance_km: float,
    ...
    """
    Рассчитывает чистую выгоду с учётом расхода топлива на дорогу.
    """
...
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # ========== ИСПОЛЬЗУЕМ НОВУЮ ФУНКЦИЮ С BOUNDING BOX ==========
            ...
            # Дальше идёт фильтрация по расстоянию и получение цен
            ...
            for station in stations:
                ...
                if dist > 20:  # Отфильтровываем слишком далёкие (хотя Bounding Box уже ограничил)
                    continue
