# handlers/find.py — ПОЛНАЯ ВЕРСИЯ (все изменения)
import html
import logging
import io
import qrcode
...
def calculate_true_savings(
    ...
    is_worth = net_savings > 10.0  # порог ощутимой выгоды
...
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not results:
                await message.answer("Не найдено АЗС с актуальными ценами. Попробуйте позже или смените топливо.")
                await state.clear()
                return
...
# ---------- ОТОБРАЖЕНИЕ КАРТОЧКИ (ПРЕМИУМ-ВЁРСТКА) ----------
...
# ---------- ОБРАБОТЧИК "ПОКАЗАТЬ ЕЩЁ 2 ВАРИАНТА" (с защитой от пустого среза) ----------
...
# ---------- СООБЩИТЬ ЦЕНУ (С АНТИСПАМОМ И ВАЛИДАЦИЕЙ) ----------
...
async def process_report_price(message: types.Message, state: FSMContext):
    # Проверка, не нажата ли кнопка меню
    ...
    async with AsyncSessionLocal() as db:
        ...
        # 2. Антиспам (не чаще 1 раза в сутки на эту АЗС)
        ...
        # 3. Отклонение от средней по городу (не более 20%)
        fuel_type = FuelType.AI_95  # можно расширить
...
async def follow_price(callback: CallbackQuery):
    ...
    await callback.answer()
    # Очищаем FSM перед обработкой
    state = FSMContext(callback.message, callback.bot)
    await state.clear()
...
async def subscribe_availability(callback: CallbackQuery):
