# handlers/find.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ (с корректным поиском и статусами)
import html
import logging
import io
import qrcode
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            for station in stations:
                ...
                # Если точной цены нет — ставим среднюю по городу, чтобы не скрывать станцию
                price_val = price_rec.price if price_rec else avg_price
                price_time = price_rec.recorded_at if price_rec else datetime.now(timezone.utc)

                dist = haversine_distance(search_lat, search_lon, station.latitude, station.longitude)

                # Создаём фиктивный объект FuelPrice для расчёта рейтинга
                if not price_rec:
                    price_rec = FuelPrice(price=price_val, recorded_at=price_time)
