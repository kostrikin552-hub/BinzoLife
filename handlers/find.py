# handlers/find.py — ПОЛНАЯ ВЕРСИЯ С ГЕОЛОКАЦИЕЙ
import html
import logging
import io
import qrcode
...
def calculate_true_savings(
    ...
    if avg_city_price <= 0 or station_price <= 0 or consumption_per_100km <= 0:
        return {
            "gross_savings": 0,
            "trip_cost": 0,
            "net_savings": 0,
...
# ---------- ОСНОВНАЯ ФУНКЦИЯ ПОИСКА (с поддержкой геолокации) ----------
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        # Получаем сохранённые координаты из состояния
        lat = data.get("lat")
        lon = data.get("lon")
...
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext, fuel_type: FuelType = None):
    try:
        ...
        text += f"\n\n💰 Цена ({fuel_type_str}): {price_text_formatted}"
...
async def handle_location(message: types.Message, state: FSMContext):
    """
    Обработчик геолокации: получает координаты, определяет город,
    и запускает поиск АЗС.
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user:
            user = await create_user(db, user_id, message.from_user.username)

        if not user.city_id:
            city = await find_nearest_city(db, lat, lon)
            if city:
                user.city_id = city.id
                await db.commit()
                await message.answer(
                    f"📍 Определён ваш город: <b>{city.name}</b>\n"
                    "Теперь я буду показывать заправки рядом с вами!",
                    parse_mode="HTML"
                )
            else:
                await message.answer(
                    "❌ Не удалось определить ваш город по координатам.\n"
                    "Пожалуйста, выберите город вручную в профиле.",
                    reply_markup=main_menu_keyboard()
                )
                return
        ...
        if not city:
            await message.answer(
                "❌ Ваш город не найден. Установите город в профиле.",
                reply_markup=main_menu_keyboard()
            )
            return
        ...
        is_live = getattr(message.location, "live_period", None) is not None
        if is_live:
            await message.answer(
                "🔄 <b>Отлично! Трансляция геопозиции активна.</b>\n\n"
                "Теперь я буду показывать заправки рядом с вами в реальном времени.\n"
                "Нажимайте «⛽ Найти заправку» для обновления результатов.",
                parse_mode="HTML"
            )
        ...
        await message.answer(
            ...
            reply_markup=fuel_choice_keyboard()
        )
