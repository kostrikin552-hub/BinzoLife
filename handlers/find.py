# handlers/find.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (компактная сетка, виральный пейволл, B2B)
import html
import logging
import io
import asyncio
...
def calculate_true_savings(
    ...
    if avg_city_price is None or avg_city_price <= 0 or station_price <= 0:
        return {"gross_savings": 0, "trip_cost": 0, "net_savings": 0,
...
def _sync_generate_share_image(
    name: str, price: float, status: str, address: str,
    ref_link: str, savings_badge: str = None
) -> bytes:
    try:
...
async def show_station_card(
    ...
    try:
        ...
        # Партнёрская ссылка передаётся в клавиатуру
        partner_url = B2B_PARTNER_LINK if (SHOW_B2B_PARTNER and index == 0) else None
