...
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext):
    try:
        ...
        # Получаем или инкрементируем просмотры
        async with AsyncSessionLocal() as db:
        ...
        text += f"\n💰 Цена: {price:.2f} ₽/л (средняя по городу: {avg_price:.2f} ₽/л → вы экономите {price_diff:.2f} ₽/л)\n"
        text += f"🕒 Данные обновлены: {price_time_str}\n"
        text += f"{views_text}"
...
async def start_report_price(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "✏️ Введите актуальную цену на {название АЗС} (в рублях за литр, например, 52.30):\n\n"
        "(Если хотите сообщить статус наличия, добавьте через пробел: цена статус, например: 52.30 GREEN)",
