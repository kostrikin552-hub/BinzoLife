...
from utils.geocoder import reverse_geocode  # <-- ДОБАВЛЕН ИМПОРТ
...
# ---------- Функция отображения карточки (с интеграцией reverse_geocode) ----------
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext):
    try:
        ...
        cleaned_address = clean_address(raw_address, max_length=255)

        # ===== НОВАЯ ЛОГИКА: если адрес пустой или мусорный, пробуем геокодер =====
        if not cleaned_address or cleaned_address.startswith("Аи-") or cleaned_address.startswith("·"):
            if station.latitude != 0.0 and station.longitude != 0.0:
                try:
                    geo_address = await reverse_geocode(station.latitude, station.longitude)
                    if geo_address:
                        cleaned_address = geo_address
                        logger.info(f"Адрес получен через геокодер для станции {station.id}: {geo_address}")
                except Exception as e:
                    logger.warning(f"Ошибка обратного геокодирования для станции {station.id}: {e}")

        station_address = html.escape(cleaned_address) if cleaned_address else "адрес не указан"
