# handlers/find.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (виральный пейволл, B2B-реклама, сочный шеринг)
import html
import logging
import io
import asyncio
...
from database.models import (
    FuelType, AvailabilityStatus, SourceType, Station,
    FuelPrice, AvailabilityReport, UserAction, Referral
)
...
# ===== Флаг включения B2B-партнёрской рекламы в карточках =====
SHOW_B2B_PARTNER = True
B2B_PARTNER_LINK = "https://t.me/your_partner_channel"  # заменить на реальную ссылку
...
def calculate_true_savings(
    ...
    if avg_city_price is None or avg_city_price <= 0 or station_price <= 0:
        return {
            "gross_savings": 0, "trip_cost": 0, "net_savings": 0,
            "is_worth": False, "badge": "Цены не подтверждены"
        }

    if consumption_per_100km <= 0:
        diff = avg_city_price - station_price
        if diff < 0:
            overpay = abs(diff) * tank_volume
            return {"gross_savings": 0, "trip_cost": 0, "net_savings": -overpay,
                    "is_worth": False, "badge": f"⚠️ Цена выше средней: переплата ~{overpay:.0f} ₽ на бак"}
        gs = diff * tank_volume
        return {"gross_savings": round(gs, 1), "trip_cost": 0, "net_savings": round(gs, 1),
                "is_worth": gs > 10.0, "badge": f"✅ Экономия: +{gs:.0f} ₽ на баке"}
    ...
    gross_savings = diff * tank_volume
    round_trip_km = distance_km * 2.0
    fuel_spent = (round_trip_km / 100.0) * consumption_per_100km
    travel_cost = fuel_spent * station_price
    ...
    return {
        "gross_savings": round(gross_savings, 1),
        "trip_cost": round(travel_cost, 1),
...
async def get_fuel_status(db, station_id: int, fuel_type: FuelType) -> dict:
    ...
    if row and row.availability != "unknown":
        return {
            "price": row.price, "availability": row.availability,
            "queue_level": row.queue_level, "observed_at": row.observed_at,
            "source": row.source or "Радар MultiGo"
        }
    ...
    return {
        ...
        "observed_at": price_rec.recorded_at if price_rec else None,
        "source": source_val
    }


# ========== ВИРАЛЬНАЯ КАРТИНКА ДЛЯ ШЕРИНГА ==========
def _sync_generate_share_image(
    name: str,
    price: float,
    status: str,
    address: str,
    ref_link: str,
    savings_badge: str = None
) -> bytes:
    try:
        img = Image.new('RGB', (800, 420), color='#121826')
        draw = ImageDraw.Draw(img)
        try:
            font_title = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 28)
            font_savings = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 26)
        ...
        draw.text((40, 30), f"⛽ {name[:30]}", fill='#FFFFFF', font=font_title)

        if savings_badge and "Экономия" in savings_badge:
            draw.rectangle([(40, 75), (700, 125)], fill='#10B981')
            draw.text((50, 85), f"🔥 {savings_badge[:60]}", fill='#FFFFFF', font=font_savings)
        else:
            draw.rectangle([(40, 75), (700, 125)], fill='#2563EB')
            draw.text((50, 85), f"💰 Честная цена: {price:.2f} ₽/л", fill='#FFFFFF', font=font_savings)

        draw.text((40, 150), f"📍 Адрес: {address[:48]}", fill='#94A3B8', font=font_text)
        draw.text((40, 190), f"🟢 Статус колонок: {status}", fill='#F8FAFC', font=font_text)
        draw.text((40, 240), "Проверено через независимый радар цен", fill='#64748B', font=font_small)
        draw.text((40, 270), "Сканируй QR или заходи в @BinzoLife_bot", fill='#38BDF8', font=font_text)
        ...
        img.paste(qr_img, (580, 200))

        buf = io.BytesIO()
        img.save(buf, format='PNG')
        return buf.getvalue()
    except Exception as e:
        logger.error(f"Ошибка генерации виральной картинки: {e}")
        return None
...
async def generate_share_image(
    ...
    return await loop.run_in_executor(
        None, _sync_generate_share_image,
        name, price, status, address, ref_link, savings_badge
    )
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not user.city_id:
            await message.answer(
                "❌ Город не выбран. Установите его в профиле.",
        ...
        if not city or city.latitude is None or city.longitude is None:
            await message.answer("❌ У города не заданы координаты.", reply_markup=main_menu_keyboard())
            await state.clear()
            return
...
async def handle_text_in_sort_state(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer(
        "⏳ Выбор сортировки отменён. Начните поиск заново.",
        reply_markup=main_menu_keyboard()
    )


@router.message(FindStates.choosing_sort)
async def handle_any_in_sort_state(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("⏳ Выбор сортировки отменён.", reply_markup=main_menu_keyboard())
...
# ========== ОСНОВНАЯ ФУНКЦИЯ ПОИСКА (с ВИРАЛЬНЫМ ПЕЙВОЛЛОМ) ==========
async def perform_search(message: types.Message, state: FSMContext, user_telegram_id: int = None):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            search_lat, search_lon = city.latitude, city.longitude
            if user.last_lat and user.last_lon:
                if haversine_distance(user.last_lat, user.last_lon, city.latitude, city.longitude) <= 80.0:
                    search_lat, search_lon = user.last_lat, user.last_lon
            ...
            if not is_pro:
                ...
                if not can_search:
                    limit_kb = InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="⚡ PRO на 24 часа (15 ⭐️ Stars)", callback_data="buy_tariff_pro_24h")],
                        [InlineKeyboardButton(text="👑 PRO на месяц (99 ₽)", callback_data="buy_tariff_pro_1m")],
                        [InlineKeyboardButton(
                            text="🎁 Получить +3 дня PRO бесплатно",
                            switch_inline_query=f"ref_{user.referral_code}"
                        )],
                        [InlineKeyboardButton(text="◀️ В главное меню", callback_data="back_to_menu")]
                    ])
                    await message.answer(
                        "🛑 <b>Бесплатный поиск на сегодня исчерпан</b>\n\n"
                        "Водители экономят до 450 ₽ с каждого полного бака благодаря точным ценам "
                        "и учёту расхода на дорогу.\n\n"
                        "<b>Как продолжить поиск прямо сейчас:</b>\n"
                        "1️⃣ <b>Оформить PRO</b> (от 15 ⭐️ Stars или 99 ₽/мес) — безлимит, графики цен и уведомления.\n"
                        "2️⃣ <b>Без денег:</b> нажмите «🎁 Получить +3 дня PRO бесплатно» и отправьте ссылку другу. "
                        "Как только он сделает 1 поиск — вам обоим начислится <b>+3 дня безлимита</b>!",
            ...
            for station in stations:
                ...
                # Считаем экономию сразу, чтобы потом показать в шеринге
                async with AsyncSessionLocal() as db2:
                    u = await get_user(db2, target_user_id)
                    tank_vol = u.tank_volume if u else 50
                sd = calculate_true_savings(
                    station_price=price_rec.price, avg_city_price=avg_price,
                    distance_km=dist, tank_volume=tank_vol
                )
                ...
                results.append({
                    ...
                    "avg_price": avg_price, "no_price": False, "savings_data": sd,
                })
            ...
            if is_fallback:
                await message.answer("⚠️ В радиусе 35 км заправок не найдено. Показываем ближайшие доступные АЗС в черте города.")
...
# ========== ОТОБРАЖЕНИЕ КАРТОЧКИ (с B2B-рекламой) ==========
async def show_station_card(
    ...
    try:
        ...
        rank_badges = {1: "🥇 Лучшая цена", 2: "🥈 Вариант №2", 3: "🥉 Вариант №3"}
        ...
        async with AsyncSessionLocal() as db:
            if price is None or price == 0.0:
                ...
                if fuel_status["price"] and fuel_status["price"] > 0:
                    price = fuel_status["price"]
                    price_time = fuel_status["observed_at"]
            else:
                fuel_status = await get_fuel_status(db, station.id, fuel_type)
                if fuel_status["price"] and fuel_status["price"] > 0:
                    if fuel_status["observed_at"] and (not price_time or fuel_status["observed_at"] > price_time):
                        price = fuel_status["price"]
                        price_time = fuel_status["observed_at"]
            ...
            queue_text = {"low": "🟢 Свободно (до 3 машин)", "medium": "🟡 Небольшая очередь (4–8 машин)",
                          "high": "🔴 Большая очередь (9+ машин)", "unknown": "⚪ неизвестно"}
            ...
            queue_str = queue_text.get(fuel_status.get("queue_level", "unknown"), "⚪ неизвестно")
        ...
        if no_price:
            price_text_formatted = "❌ Цена не подтверждена"
        elif price is not None and price > 0:
            price_text_formatted = f"{price:.2f} ₽"
        else:
            price_text_formatted = "нет данных"

        if no_price:
            rating_display = "❌"
        else:
            stars = round(rating / 20, 1) if rating else 0
            rating_display = f"⭐ {stars} ({rating}/100)"
        ...
        savings_data = {}
        if not no_price and avg_price and avg_price > 0 and price and price > 0 and distance_km > 0:
        ...
        distance_text = f"{distance_km:.1f} км" if distance_km > 0 and distance_km < 1000 else "?"
        time_min = round(distance_km / 40 * 60) if distance_km > 0 else 0
        time_text = f"~{time_min} мин" if time_min > 0 else ""
        ...
        if not no_price and savings_data.get("net_savings", 0) != 0:
            text += f"\n\n💡 {savings_data['badge']}" if savings_data.get("is_worth") else f"\n\n⚠️ {savings_data['badge']}"

        # ========== B2B-ПАРТНЁРСКАЯ РЕКЛАМА ==========
        partner_button = None
        if SHOW_B2B_PARTNER and index == 0:
            text += (
                "\n━━━━━━━━━━━━━━━━━━━━\n"
                "🤝 <b>Партнёр рядом: Автомойка «24/7 Экспресс»</b>\n"
                "<i>Скидка 20% на комплекс при заправке на этой АЗС по промокоду «БИНЗО»</i>"
            )
            partner_button = InlineKeyboardButton(
                text="🎁 Забрать скидку на мойку (20%)",
                url=B2B_PARTNER_LINK
            )
        ...
        if not is_pro:
            async with AsyncSessionLocal() as db:
                ...
                if user:
                    ...
                    if remaining > 0:
                        text += f"\n\n🔍 Осталось бесплатных поисков: {remaining} из 1."
                    else:
        ...
        # Партнёрская кнопка — второй строкой
        if partner_button:
            keyboard.inline_keyboard.insert(1, [partner_button])
...
async def show_more_stations(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        for i, res in enumerate(more_results, start=next_index+1):
            ...
            station_name = html.escape(station.name)
            map_link = f"<a href='https://yandex.ru/maps/?pt={station.longitude},{station.latitude}&z=15'>Маршрут</a>" if station.latitude else "нет координат"
            dist_text = f"{distance:.1f} км" if distance > 0 else "?"
            price_text = f"{price:.2f} ₽" if price is not None else "нет данных"
            text += f"{i}. {station_name} — {price_text}, {dist_text}\n   🗺 {map_link}\n\n"
        ...
        if new_index < len(all_results):
            kb_buttons.append([InlineKeyboardButton(text="🔍 Показать ещё 2", callback_data=f"more_{station.id}")])
...
async def back_to_best(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if all_results:
            ...
            await callback.message.answer("Нет результатов.", reply_markup=main_menu_keyboard())
            await state.clear()
    except Exception as e:
...
async def start_report_price(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"✏️ Введите актуальную цену для {fuel_code} (например, 68.50):",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def process_report_price(message: types.Message, state: FSMContext):
    if message.text in ["◀️ Назад", "/start", "/help", "📍 Найти заправку", "👤 Профиль", "⛽ Найти заправку"]:
        await state.clear()
        await message.answer("Ввод цены отменён.")
        return
    ...
    if not (35.0 <= price <= 150.0):
        await message.answer("❌ Цена должна быть от 35 до 150 ₽.")
        return
    ...
    async with AsyncSessionLocal() as db:
        ...
        recent_report = await db.execute(
            select(FuelPrice).where(
                FuelPrice.station_id == station_id,
                FuelPrice.updated_by_user_id == user.id,
                FuelPrice.recorded_at >= cutoff
            ).order_by(FuelPrice.recorded_at.desc()).limit(1)
        )
        if recent_report.scalar_one_or_none():
            await state.clear()
            await message.answer("ℹ️ Вы уже обновляли цену за последние 24 часа.")
            return

        fuel_type = FuelType(fuel_code)
        avg = await get_avg_price_30d(db, user.city_id, fuel_type)
        if avg and abs(price - avg) / avg > 0.2:
            await state.clear()
            await message.answer(f"⚠️ Цена {price:.2f} сильно отличается от средней ({avg:.2f}).")
            return

        await save_price(db, station_id=station_id, fuel_type=fuel_type, price=price,
                         source=SourceType.USER, confidence=0.6, user_id=user.id)
        if status:
    ...
    await state.clear()
    await message.answer(
        "🎉 <b>Спасибо за вклад в общее дело!</b>\n\n"
        "⭐️ Вам начислен <b>+1 балл репутации</b>.",
        parse_mode="HTML",
        reply_markup=main_menu_keyboard()
    )
...
async def follow_price(callback: CallbackQuery, state: FSMContext):
    ...
    if not await check_pro(callback.from_user.id):
        await callback.message.answer(
            "⛔ <b>Уведомления доступны только для PRO-подписчиков</b>",
            reply_markup=pro_purchase_keyboard(), parse_mode="HTML"
        )
        return
    ...
    async with AsyncSessionLocal() as db:
        ...
        for n in existing:
            if n.station_id == station_id and n.fuel_type == fuel_type and n.notify_on_low_price:
                await callback.message.answer(f"Вы уже подписаны ({fuel_type_str}).")
                return
        latest_price = await get_latest_fresh_price(db, station_id, fuel_type)
        if not latest_price:
            await callback.message.answer("Не удалось получить цену.")
            return
        target_price = max(0, round(latest_price.price - 0.5, 2))
        await create_notification(db, user_id=user.id, fuel_type=fuel_type, station_id=station_id,
                                  target_price=target_price, notify_on_low_price=True)
        await callback.message.answer(
            f"✅ Подписка на АЗС <b>{station.name}</b> активирована. Я сообщу при ≤ {target_price} ₽.",
            parse_mode="HTML"
        )
    ...
    async with AsyncSessionLocal() as db:
        ...
        for n in existing:
            if n.station_id == station_id and n.fuel_type == fuel_type and n.notify_on_availability:
                await callback.message.answer(f"Вы уже подписаны на появление {fuel_type.value}.")
                return
        await create_notification(db, user_id=user.id, fuel_type=fuel_type,
                                  station_id=station_id, notify_on_availability=True)
        await commit_or_rollback(db)
    await callback.message.answer(f"🔔 Уведомления о появлении {fuel_type.value} активированы.")
...
async def show_graph(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if graph_bytes:
            ...
            await callback.message.answer(
                f"📊 Недостаточно данных для графика {fuel_type_str}.",
                reply_markup=kb
            )
    except Exception as e:
        logger.error(f"Ошибка графика: {e}", exc_info=True)
...
async def share_station(callback: CallbackQuery, state: FSMContext):
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Формируем плашку экономии для виральной картинки
        savings_badge = None
        for res in all_results:
            if res.get("station") and res["station"].id == station_id:
                sd = res.get("savings_data", {})
                if sd.get("net_savings", 0) > 0:
                    savings_badge = f"Экономия: +{sd['net_savings']:.0f} ₽ с бака!"
                break

        ref_link = await get_referral_link(db, user)
        share_text = "Экономлю с BinzoLife — присоединяйся!"

        img = await generate_share_image(
            station.name, price, status, station.address or "", ref_link, savings_badge
        )
        if img:
            await callback.message.answer_photo(
                photo=BufferedInputFile(img, filename="share.png"),
                caption=f"📤 Поделитесь АЗС с друзьями!\n\n{share_text}\n\nВаша ссылка: {ref_link}"
            )
        else:
            await callback.message.answer(f"📤 Поделитесь!\n\n{share_text}\n\nСсылка: {ref_link}")

        today = date.today()
        share_today = await db.execute(
            select(UserAction).where(
                UserAction.user_id == user.id, UserAction.action == "share",
                func.date(UserAction.recorded_at) == today
            )
        )
        if not share_today.scalar_one_or_none():
            user.reputation += 1
            db.add(UserAction(user_id=user.id, action="share", station_id=station_id))
            await commit_or_rollback(db)
            await callback.message.answer("✅ +1 репутация (можно 1 раз в день).")
        else:
