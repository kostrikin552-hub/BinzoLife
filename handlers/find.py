# handlers/find.py – ПОЛНЫЙ (с очисткой адреса при отображении)
...
from utils.cleaners import clean_address  # <-- ИМПОРТ
...
# ---------- Функция отображения карточки (с очисткой адреса) ----------
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext):
    try:
        ...
        cleaned_address = clean_address(raw_address, max_length=255)  # <-- ОЧИСТКА
