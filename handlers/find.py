# handlers/find.py – ПОЛНАЯ ВЕРСИЯ с интеграцией is_likely_address, кеша, геокодера и лимита бесплатных поисков
...
from database.crud import (
    ...
    can_use_free_search, use_free_search  # <-- НОВЫЕ ИМПОРТЫ
)
...
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # ===== НОВАЯ ПРОВЕРКА БЕСПЛАТНЫХ ПОИСКОВ =====
            is_pro = await check_pro(user.telegram_id)
            if not is_pro:
                can_search = await can_use_free_search(db, user.id)
                if not can_search:
                    await message.answer(
                        "❌ Вы исчерпали бесплатный точный поиск на сегодня.\n"
                        "💎 Подключите PRO за 99 ₽ и получайте безлимитный доступ к точным ценам и уведомлениям.",
                        reply_markup=pro_purchase_keyboard()
                    )
                    await state.clear()
                    return
                # Используем один поиск
            ...
            logger.info(f"Средняя цена за 30 дней для {city.name}: {avg_price}")
...
# ---------- Функция отображения карточки (с интеграцией is_likely_address, кеша и геокодера) ----------
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext):
    try:
            ...
            if not is_likely_address(cleaned):
                ...
                if cached:
                    ...
                    if geo_addr:
                        ...
                        cleaned = geo_addr
                        # Обновим и саму станцию, чтобы в следующий раз не проверять
                        station.address = geo_addr
                        await db.commit()
                        logger.info(f"Адрес для станции {station.id} получен через геокодер: {geo_addr}")
                    else:
                        logger.warning(f"Геокодер не вернул адрес для станции {station.id}")
        ...
        # --- Информация о бесплатных поисках (если не PRO) ---
        if not is_pro:
            async with AsyncSessionLocal() as db:
                user = await get_user(db, message.from_user.id)
                if user:
