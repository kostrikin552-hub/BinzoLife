# handlers/find.py — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ
import html
import logging
import io
import qrcode
...
def calculate_true_savings(
    ...
    if avg_city_price is None or avg_city_price <= 0 or station_price <= 0 or consumption_per_100km <= 0:
        return {
            "gross_savings": 0,
            "trip_cost": 0,
            "net_savings": 0,
            "is_worth": False,
            "badge": "Некорректные данные"
        }
    ...
    if is_worth:
        ...
        badge = f"⚠️ Не выгодно: дорога съест {travel_cost:.0f} ₽"
    return {
...
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Если есть сохранённые координаты — используем их, иначе центр города
            if lat and lon:
                city_lat = lat
                city_lon = lon
                logger.info(f"Используем геолокацию пользователя: {city_lat}, {city_lon}")
                await save_user_location(db, user.id, lat, lon)
            else:
                city_lat = city.latitude
                city_lon = city.longitude
                logger.info(f"Используем центр города: {city_lat}, {city_lon}")

            # Сохраняем таймзону
            if user.timezone_offset is None:
                await set_user_timezone(db, user.id, city_lat, city_lon)
            ...
            # 1. Сначала ищем в радиусе 35 км от центра города или геолокации
            stations = await get_stations_in_radius(db, city.id, city_lat, city_lon, radius_km=35.0)

            # 2. Фоллбэк: если в радиусе пусто, берем все активные станции города
            if not stations:
                stations = await get_stations_by_city(db, city.id)
                if not stations:
                    await message.answer(
                        f"В г. {city.name} пока нет добавленных АЗС в базе данных.\n"
                        "Администратор может синхронизировать их командой /sync_dgis или /sync_multigo.",
                        reply_markup=main_menu_keyboard()
                    )
                    await state.clear()
                    return
            ...
            # Запрашиваем цены (включая не только сегодня, но и последние известные)
            price_stmt = (
                select(FuelPrice)
                .where(
            ...
            for a in avail_result.scalars().all():
                if a.station_id not in avails:
                    avails[a.station_id] = a
            ...
            if not results:
                await message.answer(
                    f"Не найдено АЗС с актуальными ценами для {fuel_type.value} в г. {city.name}.\n"
                    "Попробуйте выбрать другой вид топлива или обратитесь к администратору.",
                    reply_markup=main_menu_keyboard()
                )
                await state.clear()
                return

            # Засчитываем первый поиск и активируем реферальный бонус
...
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext, fuel_type: FuelType = None):
    try:
        ...
        # ========== ЦЕНА ==========
        price_text_formatted = f"{price:.2f} ₽" if price is not None else "нет данных"
        ...
        savings_data = {}
        if avg_price and avg_price > 0 and price and price is not None and price > 0 and distance_km > 0:
        ...
        text = (
            f"<b>{badge}</b> • <b>{station_name}</b> {stars_display}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
        ...
        if savings_data.get("net_savings", 0) > 0 and savings_data.get("is_worth"):
            text += f"\n\n💡 {savings_data['badge']}"
        elif savings_data.get("trip_cost", 0) > 0 and not savings_data.get("is_worth"):
...
async def share_station(callback: CallbackQuery):
