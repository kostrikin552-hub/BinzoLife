...
async def choose_fuel(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🔥 По рейтингу — идеальный баланс цены и наличия (рекомендую).\n"
        "💰 По минимальной цене — самая дешёвая.\n\n"
...
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext):
    try:
        ...
        if raw_address:
            ...
            if len(parts) >= 2:
                address_short = f"{parts[0]}, {parts[1]}"
            else:
                address_short = raw_address
        else:
            address_short = "адрес не указан"
        station_address = html.escape(address_short)
        ...
        if availability == AvailabilityStatus.GRAY:
            if availability_time:
                ...
                if age_hours > 2:
                    status_display = f"⚪ Наличие: неизвестно (данные старше {round(age_hours)} ч)"
                else:
        ...
        if distance_text != "неизвестно":
            text += f" · {distance_text}\n"
            if time_text:
                text += f"🚗 {time_text} на машине\n"
        else:
            text += "\n"

        text += f"\n💰 {price:.2f} ₽ за литр · обновлено {price_time_str}\n"
        if avg_price > 0 and abs(price_diff) > 0.01:
            if price_diff > 0:
                text += f"📊 Это на {price_diff:.2f} ₽ дешевле средней по городу\n"
            elif price_diff < 0:
                text += f"📊 Это на {abs(price_diff):.2f} ₽ дороже средней по городу\n"
        else:
        ...
        if not is_pro:
            text += (
                ...
                "Чтобы получать уведомления о появлении топлива и снижении цены в реальном времени — "
                "подключи PRO за 99 ₽/мес."
            )
...
async def show_more(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        for i, res in enumerate(more_results, start=next_index+1):
            ...
            text += (
                f"{i}. {station_name}\n"
                f"   Цена: {price:.2f} ₽, наличие: {status}, расстояние: {dist_text}\n"
                f"   🗺 {map_link}\n\n"
            )
