...
from database.crud import (
    ...
    set_user_timezone  # <-- НОВАЯ ФУНКЦИЯ ДЛЯ ТАЙМЗОНЫ
)
...
# ========== КАЛЬКУЛЯТОР ЧИСТОЙ ВЫГОДЫ (ИСПРАВЛЕННЫЙ) ==========
...
async def perform_search(message: types.Message, state: FSMContext):
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Определяем и сохраняем таймзону пользователя при первом поиске
            ...
            # ========== ЕСЛИ РЕЗУЛЬТАТОВ НЕТ — НЕ ЗАСЧИТЫВАЕМ ПОИСК ==========
            if not results:
                await message.answer("Не найдено АЗС с актуальными ценами. Попробуйте позже или смените топливо.")
                await state.clear()
                return

            # Только если есть результаты — засчитываем первый поиск и активируем реферальный бонус
            ...
            # ЗАСЧИТЫВАЕМ ПЕРВЫЙ ПОИСК ТОЛЬКО ЕСЛИ ЕСТЬ РЕЗУЛЬТАТЫ
...
async def show_station_card(message: types.Message, result: dict, index: int, total: int, is_pro: bool, state: FSMContext, fuel_type: FuelType = None):
    try:
        ...
        if avg_price and avg_price > 0 and price and price > 0 and distance_km > 0:
            async with AsyncSessionLocal() as db:
                user = await get_user(db, message.from_user.id)
