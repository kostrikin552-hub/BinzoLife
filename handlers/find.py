# handlers/find.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (исправлен callback.message.from_user.id)
import html
import logging
import io
import asyncio
...
# ========== ИСПРАВЛЕННЫЙ КАЛЬКУЛЯТОР ЧИСТОЙ ВЫГОДЫ ==========
...
async def handle_text_in_sort_state(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer(
        "⏳ Выбор сортировки был отменён. Начните поиск заново кнопкой «⛽ Найти заправку».",
        reply_markup=main_menu_keyboard()
    )
...
async def handle_any_in_sort_state(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("⏳ Выбор сортировки был отменён.", reply_markup=main_menu_keyboard())


# ========== КОЛБЭКИ СОРТИРОВКИ: ПЕРЕДАЁМ USER_TELEGRAM_ID ==========
...
# ========== ОСНОВНАЯ ФУНКЦИЯ ПОИСКА (ИСПРАВЛЕНА) ==========
async def perform_search(message: types.Message, state: FSMContext, user_telegram_id: int = None):
    """
    Основной поиск АЗС.
    user_telegram_id: явный ID пользователя (из callback.from_user.id).
    Если не передан — берётся message.from_user.id (для текстовых хендлеров).
    """
    try:
        # КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: получаем ID реального пользователя
        target_user_id = user_telegram_id if user_telegram_id else message.from_user.id
        logger.info(f"[perform_search] target_user_id={target_user_id}, from_message={message.from_user.id}, from_callback={user_telegram_id}")
        ...
        async with AsyncSessionLocal() as db:
            ...
            logger.info(f"[perform_search] user.city_id={city.id}, city.name={city.name}")

            # Используем GPS-координаты только если они ближе 80 км к центру города
            ...
            if user.last_lat and user.last_lon:
                ...
                if dist_to_city <= 80.0:
                    ...
                    has_gps = True
            logger.info(f"[perform_search] search_lat={search_lat}, search_lon={search_lon}, has_gps={has_gps}")
            ...
            if not stations:
                ...
                if not stations:
                    await message.answer(
                        ...
                        "Администратор может добавить их командой /sync_dgis или /import_city.",
                        parse_mode="HTML",
                        reply_markup=main_menu_keyboard()
                    )
                    await state.clear()
            ...
            logger.info(f"[perform_search] stations found: {len(stations)} для города {city.name}")
...
# ========== ОТОБРАЖЕНИЕ КАРТОЧКИ (ИСПРАВЛЕНО) ==========
async def show_station_card(
    ...
    try:
        ...
        # КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: используем реальный ID пользователя
        target_user_id = user_telegram_id if user_telegram_id else message.from_user.id
        ...
        # tank_volume — берём у реального пользователя
        async with AsyncSessionLocal() as db:
            user = await get_user(db, target_user_id)
        ...
        distance_text = f"{distance_km:.1f} км" if distance_km > 0 and distance_km < 1000 else "расстояние неизвестно"
        ...
        if not is_pro:
            async with AsyncSessionLocal() as db:
                ...
                if user:
                    ...
                    if remaining > 0:
                        text += f"\n\n🔍 Осталось бесплатных поисков сегодня: {remaining} (из 1)"
                    else:
                        text += "\n\n🔍 Бесплатный поиск на сегодня использован. Обновится завтра в 00:00."
...
async def show_more_stations(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if not all_results or current_index >= len(all_results):
            await callback.answer("Больше заправок с выбранным топливом не найдено ⛽", show_alert=True)
            return
        ...
        for i, res in enumerate(more_results, start=next_index+1):
            ...
            status = res.get("availability", AvailabilityStatus.GRAY)
            status = status.value if status else "GRAY"
            station_name = html.escape(station.name)
            map_link = f"<a href='https://yandex.ru/maps/?pt={station.longitude},{station.latitude}&z=15'>Маршрут</a>" if station.latitude and station.longitude else "Координаты отсутствуют"
            ...
            text += f"{i}. {station_name} — {price_text}, {status}, {dist_text}\n   🗺 {map_link}\n\n"
...
async def start_report_price(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Ошибка парсинга station_id: {e}")
...
async def process_report_price(message: types.Message, state: FSMContext):
    ...
    if not (35.0 <= price <= 150.0):
        await message.answer("❌ Цена должна быть от 35 до 150 ₽. Проверьте ввод.")
        return
    ...
    async with AsyncSessionLocal() as db:
        ...
        if recent_report.scalar_one_or_none():
            await state.clear()
            await message.answer("ℹ️ Вы уже обновляли цену на этой АЗС за последние 24 часа.")
            return
        ...
        if avg and abs(price - avg) / avg > 0.2:
            await state.clear()
            await message.answer(f"⚠️ Цена {price:.2f} сильно отличается от средней ({avg:.2f}). Проверьте данные.")
            return
    ...
    await message.answer(
        ...
        "⭐️ Вам начислен <b>+1 балл водительской репутации</b>.",
        parse_mode="HTML",
        reply_markup=main_menu_keyboard()
    )
    ...
    async with AsyncSessionLocal() as db:
        ...
        for n in existing:
            if n.station_id == station_id and n.fuel_type == fuel_type and n.notify_on_low_price:
                await callback.message.answer(f"Вы уже подписаны на эту АЗС ({fuel_type_str}).")
                return
        ...
        await callback.message.answer(
            f"✅ Подписка на АЗС <b>{station.name}</b> ({fuel_type_str}) активирована. Я сообщу при цене ≤ {target_price} ₽.",
            parse_mode="HTML"
        )
    ...
    async with AsyncSessionLocal() as db:
        ...
        for n in existing:
            if n.station_id == station_id and n.fuel_type == fuel_type and n.notify_on_availability:
                await callback.message.answer(f"Вы уже подписаны на появление {fuel_type_str}.")
                return
...
async def show_graph(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if graph_bytes:
            ...
            await callback.message.answer(
                f"📊 Недостаточно данных для графика по {fuel_type_str}.\n"
                "Для этой АЗС пока нет истории за 30 дней.",
                reply_markup=kb
            )
    except Exception as e:
