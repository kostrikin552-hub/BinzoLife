...
async def send_pro_onboarding_reminders(bot: Bot):
    ...
    которые зарегистрировались 2-3 дня назад, но ещё не оформили подписку.
    """
    try:
        async with AsyncSessionLocal() as db:
            ...
            for u in users:
                msg = (
                    ...
                    "Вам доступны:\n"
                    "🔹 <b>Пятничный радар</b> — лучшие цены перед выходными\n"
                    "🔹 <b>Экстренный поиск</b> — ближайшие АЗС в 1 клик\n"
                    "🔹 <b>Мониторинг наличия</b> и очередей в реальном времени\n\n"
                    "🚀 <i>Оформите подписку в меню /profile</i>"
                )
                try:
...
async def pro_reminder_worker(bot: Bot):
    """
    Фоновый воркер для main.py (запуск проверки раз в 24 часа).
    """
    logger.info("[ProNotifications] Воркер напоминаний PRO запущен.")
    ...
    while True:
        try:
            ...
            logger.error(f"[ProNotifications] Ошибка воркера: {e}")
