...
async def generate_price_graph(station_id: int, fuel_type: FuelType, days: int = 30) -> Optional[bytes]:
    """
    Генерирует упрощённый график цены за указанное количество дней.
    Возвращает bytes с изображением PNG или None, если данных недостаточно.
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Фильтруем аномалии (цены > 200 или < 0) — они могут быть ошибками парсинга
        ...
        # Форматирование дат на оси X – только день.месяц, без часов
        import matplotlib.dates as mdates
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%d.%m'))
        # Автоматически выбираем интервал между метками (чтобы не было слишком много)
