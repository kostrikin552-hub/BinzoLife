# services/graphics.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (без демо-данных для PRO)
import asyncio
import matplotlib
matplotlib.use('Agg')
import io
...
async def generate_price_graph(station_id: int, fuel_type: FuelType, days: int = 30, force_demo: bool = False) -> Optional[bytes]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Если недостаточно данных – не создаём демо-график даже для PRO
        if force_demo or not history or len(history) < 2:
            logger.info(f"Недостаточно данных для станции {station_id}, история пуста или менее 2 записей")
            return None
        ...
        if not filtered or len(filtered) < 2:
            logger.info(f"Цены аномальны для станции {station_id}, недостаточно данных")
            return None
        ...
        chart_bytes = await loop.run_in_executor(
            ...
            True,
            200,
            False  # is_demo = False всегда, так как демо-данные не создаём
        )
