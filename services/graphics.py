...
def aggregate_daily_data(dates: List[datetime], prices: List[float]) -> Tuple[List[datetime], List[float]]:
    ...
    Возвращает отсортированные списки дат и цен.
    """
    if not dates:
        return [], []
    ...
    # Для каждого дня берём последнюю цену (можно среднюю – но последняя более актуальна)
    sorted_days = sorted(daily_prices.keys())
    agg_dates = [datetime.combine(day, datetime.min.time()) for day in sorted_days]
    agg_prices = [daily_prices[day][-1] for day in sorted_days]  # последняя цена за день
...
def generate_fuel_price_chart(
    ...
    """
    Генерирует премиальный инвестиционный график цен на топливо.
    """
    # Агрегируем данные по дням (убираем дубликаты и частые замеры)
    agg_dates, agg_prices = aggregate_daily_data(dates, prices)
    if len(agg_dates) < 2:
        raise ValueError("После агрегации осталось меньше 2 точек данных")

    # Пересчитываем количество дней в периоде
    ...
    # Шрифты без эмодзи – для корректного отображения на Linux
    ...
    # 4. Сглаживание кривой (только если даты строго возрастают и точек ≥ 4)
    date_nums_arr = np.array(date_nums)
    # Проверяем, что все шаги по x строго положительны (уникальные даты уже гарантированы)
    diffs = np.diff(date_nums_arr)
    if HAS_SCIPY and len(date_nums) >= 4 and np.all(diffs > 0):
        try:
            # Создаём плотную сетку для плавной линии
            ...
            # Если сплайн не удался – используем линейную интерполяцию
            logger.warning(f"Сплайн не удался, используем линейную интерполяцию: {e}")
    ...
    # 11. Акцентная точка на последней цене
    ...
    # 14. Заголовок (без эмодзи, чтобы не было проблем со шрифтами)
    # Вместо ⛽ пишем "АЗС"
    ...
    # Подзаголовок с количеством дней и записей
    total_records = len(dates)  # исходное количество записей (до агрегации)
    if total_records > days_count:
        records_info = f" ({total_records} замеров)"
    else:
        records_info = ""
    ...
    # Большая карточка текущей цены и изменения
...
async def generate_price_graph(station_id: int, fuel_type: FuelType, days: int = 30, force_demo: bool = False) -> Optional[bytes]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Если данных нет или их мало – демо
