...
async def generate_price_graph(station_id: int, fuel_type: FuelType, days: int = 30) -> Optional[bytes]:
    ...
    async with AsyncSessionLocal() as db:
        # --- ПОЛУЧАЕМ НАЗВАНИЕ СТАНЦИИ СРАЗУ (исправление ошибки greenlet_spawn) ---
        station = await get_station_by_id(db, station_id)
        station_name = station.name if station else f"АЗС #{station_id}"
        # ---------------------------------------------------------------------------
    ...
    # ---------- ВСЯ РАБОТА С БАЗОЙ ДАННЫХ ЗАВЕРШЕНА ----------
    # Дальше идёт только построение графика, не требующее доступа к БД.
