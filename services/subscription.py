# services/subscription.py — ПОЛНАЯ ФИНАЛЬНАЯ ВЕРСИЯ (исправлены SQL-запросы и datetime)
import asyncio
import logging
...
async def check_expiring_subscriptions(bot: Bot):
    try:
        async with AsyncSessionLocal() as db:
            ...
            # Предупреждение за 3 дня (по pro_until)
            stmt_3d = text("""
            ...
            # Отключение истекших подписок (обновляем pro_until, не трогаем is_pro)
            stmt_expired = text("""
                UPDATE users 
                SET pro_until = NULL 
            ...
            # Уведомление об окончании триала (за 24 часа)
