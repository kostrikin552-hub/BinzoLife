# services/selfcheck.py – ПОЛНАЯ АВТОМАТИЗИРОВАННАЯ ПРОВЕРКА ВСЕХ СИСТЕМ
# Включая проверку истечения подписок, триалов, бонусных дней и автопродления
...
class SelfTester:
    def __init__(self):
        ...
        # Отдельный пользователь для проверки истечения подписки
    ...
    async def run_all(self) -> SelfCheckResult:
        ...
        # 8. Эмуляция поиска и активация триала
        ...
        # 13. Проверка движка уведомлений
        await self._check_notifications_engine()

        # 14. Проверка парсера цен
        ...
        # 18. Проверка расчёта рейтинга
        await self._check_rating()

        # 19. Проверка геокодера
        await self._check_geocoder()

        # 20. Проверка генерации графика цен
        await self._check_graphics()

        # 21. Проверка реферальной системы
        await self._check_referral_system()

        # 22. Проверка достижений и уровней
        await self._check_achievements_and_levels()

        # 23. Проверка счётчика просмотров АЗС
        await self._check_station_views()

        # 24. Проверка генерации изображения для шеринга
        await self._check_share_image_generation()

        # 25. Проверка настроек тишины
        await self._check_silent_hours()

        # 26. Проверка статистики пользователя
        await self._check_user_stats()

        # 27. Проверка экстренного поиска (без реального адреса)
        await self._check_emergency_search()

        # 28. Проверка работы автоворонки (без отправки)
        await self._check_funnel_logic()

        # ========== НОВЫЕ ПРОВЕРКИ ==========
        # 29. Проверка истечения PRO-подписки и автопродления
        await self._check_pro_expiry_and_renewal()

        # 30. Проверка истечения триала
        await self._check_trial_expiry()

        # 31. Проверка начисления и использования бонусных дней
        await self._check_bonus_days()

        # 32. Проверка деактивации истекших PRO (фоновая задача)
        await self._check_disable_expired_pro()

        # 33. Проверка получения списка пользователей с истекающей PRO
        await self._check_expiring_soon()

        # 34. Проверка очистки тестовых данных (включая второго пользователя)
        await self._cleanup()

        return self.result

    # ---------- Существующие методы (сокращённо, чтобы не дублировать) ----------
    ...
    async def _emulate_search(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if user2.funnel_stage == 1 and user2.trial_used:
                ...
                self.result.add("Эмуляция поиска", False, f"Стадия воронки = {user2.funnel_stage}, триал = {user2.trial_used}, ожидалось 1 и True", time.time() - start)
    ...
    async def _check_referral_system(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not link or "ref_" not in link:
                self.result.add("Реферальная система", False, "Не удалось получить реферальную ссылку", time.time() - start)
                return
            test_tid2 = 888888888
    ...
    async def _check_pro_expiry_and_renewal(self):
        """Проверка истечения PRO-подписки и автопродления"""
        start = time.time()
        # Создаём отдельного пользователя для теста
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Устанавливаем PRO с истечением через 1 день
            ...
            user.auto_renew = True
            await db.commit()
            # Проверяем, что пользователь в списке истекающих (get_users_expiring_soon должен вернуть его)
    ...
    async def _check_trial_expiry(self):
        """Проверка истечения триала (триал даёт PRO на 3 дня)"""
        start = time.time()
        # Используем тестового пользователя, у которого уже активирован триал (в _emulate_search)
        # Добавим проверку, что поле trial_used установлено и pro_until соответствует дате окончания триала.
        # Но у нас триал уже активирован, и pro_until = now+3 дня.
        # Проверим, что разница между pro_until и now примерно 3 дня.
        async with AsyncSessionLocal() as db:
            user = await get_user(db, TEST_TELEGRAM_ID)
            if not user:
            ...
            if abs(diff.total_seconds() - 3*24*3600) < 3600:  # плюс-минус час
                self.result.add("Истечение триала", True, f"Триал активен, окончание через {diff.days} дней", time.time() - start)
            else:
                self.result.add("Истечение триала", False, f"Некорректная длительность триала: {diff.total_seconds()/3600:.1f} ч, ожидалось ~72 ч", time.time() - start)

    async def _check_bonus_days(self):
        """Проверка начисления бонусных дней (например, через add_free_pro_days)"""
        start = time.time()
        async with AsyncSessionLocal() as db:
            user = await get_user(db, TEST_TELEGRAM_ID)
            if not user:
                ...
                return
            # Сохраним текущий pro_until
            old_pro_until = user.pro_until
            # Начислим 5 бонусных дней
            await add_free_pro_days(db, user, 5)
            # Проверим, что pro_until увеличился на 5 дней
            user2 = await get_user(db, TEST_TELEGRAM_ID)
            if user2.pro_until is not None and old_pro_until is not None:
                ...
                # Если pro_until не было (не должно быть), то просто проверим, что стало = now+5
    ...
    async def _check_disable_expired_pro(self):
        """Проверка деактивации истекших PRO (вызов disable_expired_pro)"""
        start = time.time()
        # Создаём пользователя с истекшим PRO
        ...
        async with AsyncSessionLocal() as db:
            ...
            await db.commit()
            # Вызываем disable_expired_pro
            count = await disable_expired_pro(db)
            # Проверяем, что у пользователя is_pro стал False
    ...
    async def _check_expiring_soon(self):
        """Проверка функции get_users_expiring_soon (автопродление)"""
        start = time.time()
        # Создаём пользователя с PRO, истекающим через 2 дня
        ...
        async with AsyncSessionLocal() as db:
            ...
            user.auto_renew = True
            await db.commit()
            # Получаем список истекающих (должен быть)
            expiring = await get_users_expiring_soon(db, days=3)
            found = any(u.id == user.id for u in expiring)
            if found:
    ...
    async def _cleanup(self):
        ...
        start = time.time()
        async with AsyncSessionLocal() as db:
            # Удаляем основного тестового пользователя
            user = await get_user(db, TEST_TELEGRAM_ID)
            if user:
                ...
                await db.execute(delete(Referral).where(or_(Referral.referrer_id == user.id, Referral.referred_user_id == user.id)))
                await db.execute(delete(UserAchievement).where(UserAchievement.user_id == user.id))
                await db.execute(delete(UserEconomy).where(UserEconomy.user_id == user.id))
            ...
            # Дополнительно удаляем возможных остаточных пользователей по ID, созданных в других проверках
            for tid in [888888888, 666666666, 555555555, 444444444]:
                await db.execute(text("DELETE FROM users WHERE telegram_id = :tid"), {"tid": tid})
            await db.commit()
