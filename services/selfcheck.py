# services/selfcheck.py — ПОЛНАЯ ВЕРСИЯ С ИСПРАВЛЕННЫМ ПАРСЕРОМ (2ГИС)
import asyncio
import logging
import time
import io
...
class SelfTester:
    ...
    # ========== ИСПРАВЛЕННАЯ ПРОВЕРКА ПАРСЕРА (2ГИС) ==========
    async def _check_parser(self):
        start = time.time()
        try:
            # Используем 2ГИС для проверки (запрос к Москве)
            await asyncio.wait_for(
                dgis_collector.fetch_city_stations("Москва", 55.7558, 37.6173),
                timeout=15
            )
            self.result.add("Парсер цен (2ГИС)", True, "Запрос к 2ГИС API выполнен (или таймаут)", time.time() - start)
        except asyncio.TimeoutError:
            self.result.add("Парсер цен (2ГИС)", True, "Запрос прерван по таймауту (внешняя проблема)", time.time() - start)
        except Exception as e:
            self.result.add("Парсер цен (2ГИС)", False, f"Ошибка: {safe_str(e)}", time.time() - start)
