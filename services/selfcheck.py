# services/selfcheck.py – ПОЛНЫЙ ФАЙЛ (с исправленным _cleanup, raw SQL, всеми проверками)
...
class SelfTester:
    ...
    # ---------- ИСПРАВЛЕННЫЙ МЕТОД _cleanup (с raw SQL и правильной очисткой) ----------
    async def _cleanup(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            user = await get_user(db, TEST_TELEGRAM_ID)
            if user:
                # Удаляем все связанные записи с помощью raw SQL, чтобы быть уверенными
                await db.execute(text("DELETE FROM notifications WHERE user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM payments WHERE user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM user_actions WHERE user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM referrals WHERE referrer_id = :user_id OR referred_user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM user_achievements WHERE user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM user_economies WHERE user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM reviews WHERE user_id = :user_id"), {"user_id": user.id})
                await db.execute(text("DELETE FROM availability_reports WHERE user_id = :user_id"), {"user_id": user.id})
                # Теперь удаляем самого пользователя
                await db.execute(text("DELETE FROM users WHERE id = :user_id"), {"user_id": user.id})
                await db.commit()
            ...
            for tid in [888888888, 666666666, 555555555, 444444444]:
                try:
                    ...
                    if user:
                        # Удаляем связанные записи для этого пользователя
                        await db.execute(text("DELETE FROM notifications WHERE user_id = :user_id"), {"user_id": user.id})
                        await db.execute(text("DELETE FROM payments WHERE user_id = :user_id"), {"user_id": user.id})
                        await db.execute(text("DELETE FROM user_actions WHERE user_id = :user_id"), {"user_id": user.id})
                        await db.execute(text("DELETE FROM referrals WHERE referrer_id = :user_id OR referred_user_id = :user_id"), {"user_id": user.id})
