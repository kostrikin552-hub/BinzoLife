# services/selfcheck.py – ИСПРАВЛЕННАЯ ВЕРСИЯ
# Добавлен импорт get_station_by_id, исправлен вызов check_pro
import asyncio
import logging
import time
import io
...
class SelfTester:
    ...
    async def _check_pro_rights(self, db: AsyncSession):
        start = time.time()
        if not self.test_telegram_ids:
            self.result.add("Проверка PRO-прав", False, "Нет telegram_id тестового пользователя", time.time() - start)
            return
        try:
            # Передаём именно telegram_id
            is_pro = await check_pro(self.test_telegram_ids[0])
            if is_pro:
                self.result.add("Проверка PRO-прав", True, "check_pro вернул True", time.time() - start)
            else:
                # Дополнительная проверка: возможно, пользователь не найден по telegram_id
                user = await get_user(db, self.test_telegram_ids[0])
                if user:
