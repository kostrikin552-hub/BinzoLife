# services/selfcheck.py – ФИНАЛЬНАЯ ВЕРСИЯ (БЕЗ ОЧИСТКИ)
...
class SelfTester:
    ...
    async def _create_test_user(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            # Удаляем только если пользователь существует, иначе может быть ошибка
            try:
    ...
    async def _check_referral_system(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Удаляем второго пользователя, но без ошибок
            try:
                await db.execute(text("DELETE FROM users WHERE telegram_id = :tid"), {"tid": test_tid2})
                await db.commit()
            except Exception:
