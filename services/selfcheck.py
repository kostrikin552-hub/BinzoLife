# services/selfcheck.py — ИСПРАВЛЕННАЯ ВЕРСИЯ (использует MultiGo)
import asyncio
import logging
import time
import io
...
# Импорт нового парсера MultiGo вместо fuelprice_parser
...
class SelfTester:
    ...
    async def run_all(self) -> SelfCheckResult:
        async with AsyncSessionLocal() as db:
            try:
                ...
                await self._check_parser()  # <-- обновлено
    ...
    # ========== ИСПРАВЛЕННАЯ ПРОВЕРКА ПАРСЕРА (MultiGo) ==========
    async def _check_parser(self):
        start = time.time()
        try:
            # Используем MultiGo для проверки (запрос к Москве)
            await asyncio.wait_for(
                multigo_parser.fetch_stations_in_bbox(55.7558, 37.6173),
                timeout=15
            )
            self.result.add("Парсер цен (MultiGo)", True, "Запрос к MultiGo API выполнен (или таймаут)", time.time() - start)
        except asyncio.TimeoutError:
            self.result.add("Парсер цен (MultiGo)", True, "Запрос прерван по таймауту (внешняя проблема)", time.time() - start)
        except Exception as e:
            self.result.add("Парсер цен (MultiGo)", False, f"Ошибка: {safe_str(e)}", time.time() - start)
