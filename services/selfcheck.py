# services/selfcheck.py – ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ
import asyncio
import logging
import time
import io
...
from database.crud import (
    ...
    get_station_by_id           # <-- ДОБАВЛЕН ИМПОРТ
)
...
class SelfTester:
    def __init__(self):
        ...
        self.test_telegram_ids = []   # храним telegram_id тестовых пользователей
        self.test_user_ids = []       # храним id из БД (для других целей)
        self.test_station_ids = []
        self.test_city_ids = []
        self.test_notification_ids = []
        self.test_payment_ids = []
        self._message_semaphore = asyncio.Semaphore(1)
        self.current_test_telegram_id = None  # текущий telegram_id для проверок

    # ---------- Безопасная отправка сообщений (защита от flood) ----------
    async def _send_message_safe(self, chat_id: int, text: str, max_retries: int = 3):
        """Отправляет сообщение с автоматическим повторением при flood control."""
    ...
    async def run_all(self) -> SelfCheckResult:
        async with AsyncSessionLocal() as db:
            try:
                ...
                await self._check_pro_rights(db)   # <-- исправленный метод
                ...
                await self._check_user_stats(db)   # <-- исправленный метод
    ...
    async def _create_test_user(self, db: AsyncSession) -> Optional[User]:
        # Генерируем ID в диапазоне, который точно не занят
        base_id = -2000000
        max_attempts = 30
        telegram_id = None
        ...
        try:
            ...
            db.add(user)
            await db.flush()
            self.test_telegram_ids.append(telegram_id)   # сохраняем telegram_id
            self.test_user_ids.append(user.id)           # сохраняем id из БД
    ...
    # ========== ИСПРАВЛЕННАЯ ПРОВЕРКА PRO-ПРАВ ==========
    async def _check_pro_rights(self, db: AsyncSession):
        ...
        if not self.test_user_ids:
            ...
            return
        try:
            # Используем id пользователя, а не telegram_id
            user_id = self.test_user_ids[0]
            is_pro = await check_pro(user_id)  # check_pro принимает id или telegram_id
            if is_pro:
    ...
    async def _check_station_views(self, db: AsyncSession):
        ...
        try:
            station_id = self.test_station_ids[0]
            station = await get_station_by_id(db, station_id)   # <-- теперь импортировано
            if station:
                today = date.today()
    ...
    # ========== ИСПРАВЛЕННАЯ СТАТИСТИКА ПОЛЬЗОВАТЕЛЯ ==========
    async def _check_user_stats(self, db: AsyncSession):
        ...
        try:
            ...
            if not user:
                ...
                return
            # Добавляем ещё один поиск, чтобы итого стало 4
            ...
            if search_count == 4:
                ...
                self.result.add("Статистика пользователя", False, f"Поисков: {search_count} (ожидалось 4)", time.time() - start)
        except Exception as e:
    ...
    async def _check_disable_expired_pro(self, db: AsyncSession):
        ...
        try:
            ...
            user.pro_until = None
            await db.flush()
            user2 = await get_user(db, self.test_telegram_ids[-1])  # используем последний созданный
            if not user2:
