# services/selfcheck.py – ПОЛНЫЙ ФАЙЛ (все проверки + исправленный _cleanup без удаления основного тестового пользователя)
...
class SelfTester:
    ...
    # ---------- ИСПРАВЛЕННЫЙ МЕТОД _cleanup (без удаления основного тестового пользователя) ----------
    async def _cleanup(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            # Проверяем, существует ли тестовый пользователь
            test_user = await get_user(db, TEST_TELEGRAM_ID)
            if test_user:
                # Мы НЕ удаляем тестового пользователя, чтобы избежать ошибок внешнего ключа
                logger.info(f"Тестовый пользователь (ID: {test_user.id}, telegram_id: {TEST_TELEGRAM_ID}) оставлен в БД для избежания ошибок внешнего ключа.")
                self.result.add("Очистка тестовых данных", True, "Тестовый пользователь оставлен (без удаления)", time.time() - start)
            else:
                self.result.add("Очистка тестовых данных", True, "Тестовый пользователь уже отсутствовал", time.time() - start)

            # Удаляем временных пользователей из других проверок
            for tid in [888888888, 666666666, 555555555, 444444444]:
                try:
                    ...
                    if user:
                        ...
                        await db.execute(text("DELETE FROM payments WHERE user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM user_actions WHERE user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM referrals WHERE referrer_id = :user_id OR referred_user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM user_achievements WHERE user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM user_economies WHERE user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM reviews WHERE user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM availability_reports WHERE user_id = :user_id"), {"user_id": user_id})
                        await db.execute(text("DELETE FROM users WHERE id = :user_id"), {"user_id": user_id})
                        await db.commit()
                        logger.info(f"Временный пользователь {tid} удалён")
                except Exception as e:
