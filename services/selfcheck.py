...
# Константы для тестов (будут заменены на случайные ID в рантайме)
...
class SelfCheckResult:
    def __init__(self):
        ...
        self.test_ids: Dict[str, int] = {}  # для хранения созданных ID
...
class SelfTester:
    def __init__(self):
        ...
        # Генерируем уникальные тестовые Telegram ID
        self.test_telegram_ids = [
            random.randint(10**9, 10**10) for _ in range(6)
        ]
        self.test_user_ids = []
    ...
    async def run_all(self) -> SelfCheckResult:
        # Запускаем все проверки в рамках одной транзакции с откатом
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                logger.error(f"Критическая ошибка в самопроверке: {e}", exc_info=True)
                self.result.add("Общая ошибка", False, str(e), time.time() - self.result.start_time)
    ...
    async def _create_test_city(self, db: AsyncSession, name: str = "Тестовый город", lat: float = 55.0, lon: float = 82.0) -> City:
        ...
        if city:
            # Если город уже существует, делаем его активным и возвращаем
            city.is_active = True
            await db.flush()
            return city
    ...
    async def _create_test_user(self, db: AsyncSession) -> User:
        # Создаём пользователя с уникальным telegram_id
        telegram_id = self.test_telegram_ids.pop()
        user = await create_user(db, telegram_id, TEST_USERNAME)
        self.test_user_ids.append(user.id)
        return user

    # ---------- Реализация всех проверок ----------
    ...
    async def _check_tables(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Наличие таблиц", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_all_cities(self, db: AsyncSession):
        ...
        try:
            ...
            if not cities:
                ...
                return
            # Проверяем, есть ли у всех городов координаты и станции
            issues = []
            for city in cities:
            ...
            self.result.add("Города в БД", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_stations_and_prices(self, db: AsyncSession):
        ...
        try:
            ...
            stations = await get_stations_by_city(db, city.id)
            if not stations:
                # Создаём тестовую станцию
                station = await self._create_test_station(db, city.id, 56.02, 92.86)
                await save_price(db, station.id, FuelType.AI_95, 65.0, SourceType.ADMIN, 0.9, recorded_at=datetime.now(timezone.utc))
                await save_availability_report_with_consensus(
                    db, station.id, FuelType.AI_95, AvailabilityStatus.GREEN,
                    SourceType.ADMIN, 0.9, recorded_at=datetime.now(timezone.utc)
                )
            # Проверяем наличие свежих цен
            ...
            self.result.add("АЗС и цены", False, f"Ошибка: {e}", time.time() - start)

    async def _create_test_user(self, db: AsyncSession):
        start = time.time()
        try:
            # Проверяем, нет ли уже такого пользователя
            user = await get_user(db, self.test_telegram_ids[0])
            if user:
                self.result.add("Создание тестового пользователя", False, "Пользователь уже существует", time.time() - start)
                return
            user = await self._create_test_user(db)
            self.result.add("Создание тестового пользователя", True, f"ID: {user.id}", time.time() - start)
        except Exception as e:
            self.result.add("Создание тестового пользователя", False, f"Ошибка: {e}", time.time() - start)

    async def _set_user_city(self, db: AsyncSession):
        start = time.time()
        try:
            user = await get_user(db, self.test_user_ids[0])
            if not user:
            ...
            self.result.add("Установка города", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _emulate_search(self, db: AsyncSession):
        ...
        try:
            ...
            if not user:
                ...
                return
            # Сбрасываем состояние для чистоты
            user.funnel_stage = 0
            ...
            db.add(action)
            # Активируем триал, если ещё не активирован
            if not user.trial_used:
                await activate_trial(db, user.id)
            await db.flush()
            # Проверяем результат
            user2 = await get_user(db, self.test_user_ids[0])
            if user2.funnel_stage == 1 and user2.trial_used and user2.pro_until is not None:
            ...
            self.result.add("Эмуляция поиска", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _create_test_notification(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Создание уведомления", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _emulate_pro_payment(self, db: AsyncSession):
        ...
        try:
            ...
            if not user:
                ...
                return
            charge_id = f"test_charge_{self.test_user_ids[0]}"
            ...
            self.result.add("Эмуляция оплаты PRO", False, f"Ошибка: {e}", time.time() - start)

    async def _check_pro_rights(self, db: AsyncSession):
        start = time.time()
        try:
            is_pro = await check_pro(self.test_user_ids[0])
            if is_pro:
            ...
            self.result.add("Проверка PRO-прав", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_notifications_engine(self):
        start = time.time()
        try:
            # Вызываем check_notifications, она не должна падать
    ...
    async def _check_parser(self):
        start = time.time()
        try:
            # Проверяем парсинг для Москвы с таймаутом, но не считаем таймаут ошибкой
    ...
    async def _check_unsubscribe(self, db: AsyncSession):
        start = time.time()
        try:
            notif_id = self.test_notification_ids[0] if self.test_notification_ids else None
            if not notif_id:
            ...
            self.result.add("Отписка от уведомления", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_admin_commands(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Админ-команды", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_referral_system(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Реферальная система", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_achievements_and_levels(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Достижения и уровни", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_station_views(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Счётчик просмотров АЗС", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_silent_hours(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Настройка тишины", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_user_stats(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Статистика пользователя", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_emergency_search(self, db: AsyncSession):
        ...
        try:
            ...
            if not city:
                city = await self._create_test_city(db, "Красноярск", 56.0109, 92.8525)
            # Проверяем наличие станции с GREEN статусом
            cutoff = datetime.utcnow() - timedelta(hours=2)
            green_station = await db.execute(
                select(Station)
            ...
            station = green_station.scalar_one_or_none()
            if not station:
                # Создаём тестовую станцию
            ...
            self.result.add("Экстренный поиск", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_pro_expiry_and_renewal(self, db: AsyncSession):
        start = time.time()
        try:
            # Создаём пользователя с PRO, истекающим через 1 день
            user = await self._create_test_user(db)
            ...
            self.result.add("Истечение PRO (список expiring)", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_trial_expiry(self, db: AsyncSession):
        ...
        try:
            ...
            if not user:
                ...
                return
            # Сбрасываем и активируем триал заново
            user.trial_used = False
            user.is_pro = False
            user.pro_until = None
            await db.flush()
            ...
            self.result.add("Истечение триала", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_bonus_days(self, db: AsyncSession):
        ...
        try:
            ...
            if user2.pro_until is not None and old_pro_until is not None:
                ...
                # Если pro_until не было, проверяем, что стало = now+5
            ...
            self.result.add("Бонусные дни", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_disable_expired_pro(self, db: AsyncSession):
        ...
        try:
            ...
            now = datetime.now(timezone.utc)
            user.is_pro = True
            user.pro_until = now - timedelta(days=1)  # истекло
            await db.flush()
            count = await disable_expired_pro(db)
            user2 = await get_user(db, user.id)
            if not user2.is_pro:
            ...
            self.result.add("Деактивация истекших PRO", False, f"Ошибка: {e}", time.time() - start)
    ...
    async def _check_expiring_soon(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("get_users_expiring_soon", False, f"Ошибка: {e}", time.time() - start)
