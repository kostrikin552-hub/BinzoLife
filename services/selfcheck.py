# services/selfcheck.py – ПОЛНЫЙ ФАЙЛ (все 34 проверки, экранирование HTML)
...
class SelfCheckResult:
    ...
    def summary(self) -> str:
        ...
        for name, success, msg, duration in self.checks:
            ...
            lines.append(f"{icon} <b>{escaped_name}</b> ({duration:.2f}с)")
            if escaped_msg:
                lines.append(f"   <i>{escaped_msg}</i>")
        return "\n".join(lines)
