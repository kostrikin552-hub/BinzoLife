# services/selfcheck.py – ПОЛНЫЙ ФАЙЛ (с исправленными методами создания пользователей)
...
class SelfTester:
    ...
    # ========== ИСПРАВЛЕННЫЙ МЕТОД _create_test_user ==========
    ...
    # ========== ИСПРАВЛЕННЫЙ МЕТОД _check_referral_system ==========
    async def _check_referral_system(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            # Проверяем, существует ли пользователь с ID 888888888
    ...
    # ========== ИСПРАВЛЕННЫЕ МЕТОДЫ ДЛЯ ВРЕМЕННЫХ ПОЛЬЗОВАТЕЛЕЙ ==========

    async def _check_pro_expiry_and_renewal(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            # Проверяем, существует ли пользователь с ID 666666666
    ...
    async def _check_disable_expired_pro(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            # Проверяем, существует ли пользователь с ID 555555555
    ...
    async def _check_expiring_soon(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            # Проверяем, существует ли пользователь с ID 444444444
