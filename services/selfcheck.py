# services/selfcheck.py – ПОЛНЫЙ ФАЙЛ (с исправленным _cleanup)
...
class SelfTester:
    ...
    # ---------- Все методы проверок (полный набор) ----------
    ...
    # ---------- ИСПРАВЛЕННЫЙ МЕТОД _cleanup (с принудительным удалением ВСЕХ связанных записей) ----------
    async def _cleanup(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if user:
                user_id = user.id
                logger.info(f"Очистка тестового пользователя ID={user_id}")

                # Принудительно удаляем все записи, ссылающиеся на этого пользователя
                # Используем сырой SQL, чтобы быть уверенными
                ...
                # Проверяем, остались ли ещё записи в notifications (на случай, если есть с другими user_id)
                ...
                if check.scalar_one_or_none() is None:
                    ...
                    self.result.add("Очистка тестовых данных", False, "Пользователь не был удалён", time.time() - start)
            else:
