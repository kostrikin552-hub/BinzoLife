# services/selfcheck.py – ИСПРАВЛЕННАЯ ВЕРСИЯ (без рекурсии в логировании)
...
def safe_str(obj: Any) -> str:
    """Безопасное преобразование объекта в строку без рекурсии."""
    if obj is None:
        return "None"
    try:
        # Если это исключение, берём только сообщение
        if isinstance(obj, Exception):
            return f"{obj.__class__.__name__}: {str(obj)}"
        # Для других объектов используем repr, чтобы избежать рекурсии
        return repr(obj)
    except Exception:
...
class SelfTester:
    ...
    async def _create_test_user(self, db: AsyncSession):
        ...
        try:
            ...
            self.result.add("Создание тестового пользователя", False, f"Ошибка: {safe_str(e)}", time.time() - start)
