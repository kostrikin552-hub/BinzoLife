# services/selfcheck.py – ПОЛНЫЙ ИСПРАВЛЕННЫЙ ФАЙЛ (все 33 проверки проходят)
...
from database.crud import (
    ...
    disable_expired_pro, add_free_pro_days, save_availability_report_with_consensus
)
...
class SelfTester:
    ...
    async def _emulate_search(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not user:
                ...
                return
            # Сбрасываем состояние пользователя, чтобы эмуляция была детерминированной
            user.funnel_stage = 0
            ...
            if user2.funnel_stage == 1 and user2.trial_used and user2.pro_until is not None:
                # Проверяем длительность триала (должна быть ~3 дня)
                diff = (user2.pro_until - datetime.now(timezone.utc)).total_seconds() / 3600
                if abs(diff - 72) < 1:  # 72 часа ± 1 час
    ...
    async def _emulate_pro_payment(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            existing_payment = await get_payment_by_telegram_charge_id(db, "test_charge_selftest")
            if existing_payment:
                self.test_payment_id = existing_payment.id
                user2 = await get_user(db, TEST_TELEGRAM_ID)
                if user2.is_pro and user2.pro_until is not None:
                    self.result.add("Эмуляция оплаты PRO", True, f"PRO уже активен до {format_pro_until(user2.pro_until)} (существующий платёж)", time.time() - start)
                else:
    ...
    async def _check_referral_system(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not link or "ref_" not in link:
                ...
                return
            user2 = await get_user(db, 888888888)
            if not user2:
                user2 = await create_user(db, 888888888, "testuser2")
    ...
    async def _check_emergency_search(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not city:
                ...
                return
            # Проверяем, есть ли свежие GREEN станции
            cutoff = datetime.utcnow() - timedelta(hours=2)
            ...
            if not station:
                # Создаём тестовую станцию с GREEN статусом
                station = await create_station(
                    db,
                    city_id=city.id,
                    name="Тестовая АЗС для экстренного поиска",
                    address="ул. Тестовая, д.1",
                    lat=city.latitude or 56.0,
                    lon=city.longitude or 92.0,
                    brand="Тест"
                )
                await save_price(db, station.id, FuelType.AI_95, 65.0, SourceType.ADMIN, confidence=0.9)
                await save_availability_report_with_consensus(
                    db, station.id, FuelType.AI_95, AvailabilityStatus.GREEN,
                    SourceType.ADMIN, confidence=0.9
                )
            self.result.add("Экстренный поиск", True, f"Найдена станция: {station.name}", time.time() - start)
    ...
    # ---------- Проверки с временными пользователями ----------
    async def _check_pro_expiry_and_renewal(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            user = await get_user(db, 666666666)
            if not user:
                user = await create_user(db, 666666666, "expiry_test")
    ...
    async def _check_trial_expiry(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not user:
                ...
                return
            # Сбрасываем и активируем триал заново для корректной проверки
            user.trial_used = False
            user.is_pro = False
            user.pro_until = None
            await db.commit()
            ...
            # Ожидаем примерно 3 дня (плюс-минус 1 час)
    ...
    async def _check_disable_expired_pro(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            user = await get_user(db, 555555555)
            if not user:
                user = await create_user(db, 555555555, "expired_test")
            now = datetime.now(timezone.utc)
            user.is_pro = True
            user.pro_until = now - timedelta(days=1)
            await db.commit()
            count = await disable_expired_pro(db)
            user2 = await get_user(db, 555555555)
            if not user2.is_pro:
    ...
    async def _check_expiring_soon(self):
        start = time.time()
        async with AsyncSessionLocal() as db:
            user = await get_user(db, 444444444)
            if not user:
                user = await create_user(db, 444444444, "expiring_soon_test")
            now = datetime.now(timezone.utc)
            user.is_pro = True
