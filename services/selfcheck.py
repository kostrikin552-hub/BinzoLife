...
# - безопасное логирование (safe_str)
# - надёжное создание тестового пользователя (отрицательные ID с проверкой)
# - обработка flood control в _send_message_safe
# - все обращения к get_user используют telegram_id, а не id
    ...
    async def _check_bot_token(self):
        ...
        try:
            ...
            if "RetryAfter" not in str(e) and "flood" not in str(e).lower():
                await self._send_message_safe(settings.ADMIN_ID, f"❌ Ошибка токена бота: {e}")
